...
- [x] 0.2 Re-derive the two parent policies rather than trusting `proposal.md`'s table. Any change
...
- [x] 0.3 Confirm `club_members`' primary key is still `(club_id, user_id)`. If it is not, the
  ...
  ```
- [x] 0.4 Record the gate-trigger count **before** the migration, so the after-count means
  something: `select count(*) from pg_trigger where tgname='enforce_participation_gate' and not
  tgisinternal;` — **17** on DEV, 2026-08-31.
- [x] 0.5 Read both `notifications` CHECK constraints verbatim; §2 rewrites them and they are the
...
- [x] 0.7 **Answer `design.md` §Q1** — the share affordance. It is the only blocking question and
  it is the objection at the top of `proposal.md`.

## 1. `092` — the two tables

- [x] 1.1 `club_thread_waves (thread_id, user_id, created_at)`, PK `(thread_id, user_id)`.
...
- [x] 1.2 `club_join_waves (club_id, subject_user_id, user_id, created_at)`, PK
...
- [x] 1.3 **An index leading with `user_id` on each table**, for the `profiles` cascade. Both
...
- [x] 1.4 `comment on table` for each, naming `postcard_likes` as the same concept under the older
...
- [x] 2.1 Widen `notifications_type_check` with `club_waved`.
- [x] 2.2 Widen `notifications_subject_shape` with a `WHEN 'club_waved'` arm requiring `club_id IS
  ...
  forgotten arm loud.
- [x] 2.3 **No `thread_id` column on `notifications`.** A thread wave notifies nobody (§Q2). Adding
  the column "for later" would owe an index, a shape arm and a cascade path.

## 3. `092` — policies and grants

- [x] 3.1 SELECT on each:
  `user_id = auth.uid() or (<parent EXISTS> and not private.is_blocked(auth.uid(), user_id))`.
  **Nothing else.** Write the comment that says why no membership, club-visibility or
  parent-author-block conjunct appears — and, per `081`'s lesson, do not justify anything as
  *"the parent alone is a leak"* unless it is.
  > **CORRECTED 2026-08-31, product owner's decision: the own-row branch is a disjunct of the
  > WHOLE policy, not of the block arm.** This task originally read "the parent `EXISTS` **plus**
  > `user_id = auth.uid() or not is_blocked(…)`", which is `postcard_likes`' shape and is wrong
  > twice: inside the block arm the branch is a **no-op**, because `blocks_no_self_block` already
  > makes `is_blocked(x, x)` false; and the parent `EXISTS` still dominates it, so §5.6 could not
  > pass. Measured before the fix: a rider blocked by a thread's author, and a rider who had merely
  > LEFT the club, both read zero of their own waves and both got `DELETE 0` with the row
  > surviving. The hoist moves **exactly four assertions in 2570 and nothing else**, and widens
  > nothing — the branch returns only rows the caller wrote. `design.md` §D7 carries the full
  > argument; `092.6` is the un-hoist detector. **Both sides of every `EXISTS` are also
  > table-qualified**, or `m.club_id = m.club_id` makes it a tautology.
- [x] 3.2 INSERT on each: `user_id = auth.uid()` plus the **same** `EXISTS`, plus
...
- [x] 3.3 DELETE on each: `using (user_id = auth.uid())`, **no visibility conjunct** (`009`'s rule).
...
- [x] 3.4 **No UPDATE policy and no UPDATE grant** on either — a wave has no mutable column.
...
- [x] 3.5 `revoke all … from anon, authenticated`, then `grant select, delete` at table level and
  `grant insert (…)` **per column**, omitting `created_at`. Nothing to `anon` — decision #1.

## 4. `092` — triggers

- [x] 4.1 `enforce_participation_gate` on both, `before insert … for each row when (current_user =
...
- [x] 4.2 `private.notify_club_waved()` — `security definer`, `set search_path = ''`, recipient
...
- [x] 4.3 `private.retract_club_waved()` — `after delete`, scoped by **all four** of `user_id`,
  `type`, `actor_id`, `club_id` (`036` §7.2). **No `pg_trigger_depth` guard.**
- [x] 4.4 Both fan-out triggers carry **no `when` clause** — `036` §7.8: copying `023`'s guard here
...
- [x] 5.1 A non-member of a **public** club reads zero `club_thread_waves` and cannot insert one.
  (Fails if the parent `EXISTS` is dropped.)
- [x] 5.2 A non-member of a **private** club reads zero of both.
- [x] 5.3 A member reads a fellow member's wave; after a block **in each direction**, reads zero and
  the **count** drops by one. Two cases, not one — the row and the aggregate.
- [x] 5.4 A rider blocked with a thread's **author** can neither read nor insert a wave on it, and
  the refusal comes from the parent rather than from a conjunct in this file.
- [x] 5.5 A rider blocked with a join's **subject** can neither read nor insert a wave on it.
- [x] 5.6 A rider **can always read and delete their own wave**, while blocked by the parent's
  author. (Fails if the SELECT own-row disjunct is removed — the delete-path assertion of §3.3.)
  > **This did NOT hold against §3.1 as originally written, and the policy was corrected rather
  > than the requirement** (product owner, 2026-08-31). `092.6` now carries three
  > `UN-HOIST DETECTOR` assertions — blocked-by-the-author reads its own row, that delete matches,
  > and a rider who has LEFT the club withdraws both waves they left behind — plus `092.4`'s
  > own-row read on an invisible thread. Verified both ways: un-hoisting the branch flips exactly
  > those four and nothing else in 2570. The third detector is the one with **no block in it at
  > all**, which is the commoner instance and the one the requirement's own wording is about.
- [x] 5.7 A club **owner** and an **admin** reach exactly what a member reaches; **no policy names
  `role`**, asserted by reading `pg_policies` rather than by two role fixtures agreeing.
- [x] 5.8 A self-wave on a join is refused; a self-wave on a thread succeeds.
- [x] 5.9 `anon` holds **no** table privilege on either table, and no policy targets a role other
  than `authenticated`.
- [x] 5.10 **Leaving a club deletes the join's waves**, and rejoining shows zero. This is §D4 and it
  is the assertion that fails against the bare-`profiles` key.
- [x] 5.11 Deleting a **reactor's** profile deletes their waves; deleting a **subject's** profile
  deletes their memberships and, by cascade, the waves on them.
- [x] 5.12 Every foreign key into `profiles` on both tables leads an index — the `pg_constraint` /
  `pg_index` form `029` uses, never a timing.
- [x] 5.13 `enforce_participation_gate` is present **by table name** on both, and the flat count is
  **19**. Both, because a count alone cannot tell a new gate from a moved one.
- [x] 5.14 The fan-out writes **exactly one** row, addressed to the joiner. Count it.
- [x] 5.15 The retraction removes exactly its own row: two wavers, one un-waves, the other's
  notification survives.
- [x] 5.16 A wave/un-wave/wave loop leaves **at most one** live notification, through
  `notifications_event_key`; assert `indnullsnotdistinct` is true rather than inferring the
  collapse.
- [x] 5.17 No UPDATE policy and no UPDATE grant on either table.
- [x] 5.18 Re-run the whole suite and **compare label sets, not counts** — a count cannot tell a
  rename from a loss.

## 6. Client

- [x] 6.1 `src/lib/data/club-waves.ts` — `attachClubWaveState`, one batched read per subject kind,
...
- [x] 6.2 Neither function restates a membership, block, club-visibility or role predicate. Say so
  in each docstring, the way `getClubThreads` does.
- [x] 6.3 `src/lib/actions/club-waves.ts` — four plain async functions. `invalidate` only the wave
  keys; **not** the thread list, whose rows have not changed.
- [x] 6.4 Two keys in `src/lib/query/keys.ts`, both children of `clubs.detail(clubId)`, each with
  the docstring that file's convention requires. **No key holding a decorated entry.**
- [x] 6.5 Extract `LikeButton`'s optimistic toggle into one shared control and re-point both
...
- [x] 6.6 `ClubTimelineEventRow` gains the control on `join` and `thread` entries only. **Not** on
...
- [x] 6.7 The zero count draws **nothing**. Check the 44px row still holds its avatar, sentence,
  unread dot, control and `Time Since` at the narrowest supported width before adding a sixth slot.
- [x] 6.8 **Say welcome** on the join row's overflow (§D3, §Q6) — a pre-filled title into
  `CreateThreadForm`, editable, writing nothing until submit. No new column, no link back.
- [x] 6.9 `notificationCopy`, `NotificationsListItem`'s `describe`, and the type union in
...
- [x] 6.10 Icons from `@/components/icons/generated` — `WaveIcon`, the existing glyph. Primary
  buttons are near-black `Grey/100`, never green.
- [ ] 6.11 **SUPERSEDED** — see `proposal.md`'s banner and design.md §Q1, ANSWERED 2026-08-31: a
  thread's ⋯ menu now carries `Share club` (`ClubThreadPage`'s `ThreadOptions`), reusing
  `shareAppLink(routes.club(clubId), ...)`. Left unchecked because the box as originally written
  ("No share affordance on a thread") is no longer the built behaviour; ticking it would misstate
  what shipped. The safety property (the label, never a thread URL) and the PD-299 #2 note are
  both in that component's docstring.
...
- [x] 8.2 A component test for the wave control asserting the **constant** accessible name across
  ...
  once already. `src/components/clubs/__tests__/ClubWaveButton.test.tsx`; verified both ways by
  temporarily reintroducing a `pressed`-branching label and confirming it goes red.
- [x] 8.3 A component test asserting the control is **absent** for a non-member, on
  `RideInviteJoin`'s precedent — the defect here is a control that renders, so an assertion that
  something renders cannot see it. Read as "absent on a rider's own join" (§Q3's self-wave refusal)
  rather than "non-member", which never reaches the row at all — a non-member is refused the
  underlying read before this component exists. `src/components/clubs/__tests__/
  ClubTimelineEventRow.test.tsx`; verified both ways by temporarily removing the `!isSelf` guard and
  confirming it goes red.
- [x] 8.4 A unit test asserting `mergeClubTimeline` is **unchanged**: the same five sources, the
  same horizon set. The regression this catches is a wave read acquiring a `horizon` and truncating
  the stream it decorates. `src/lib/data/__tests__/club-timeline.test.ts`.
- [ ] 8.5 `npx tsc --noEmit`, `npm run lint`, `npm run test:unit`, `npm run build`. First three
  clean; `test:unit` is 2677/2680 — the 3 failures are all in `crossrefs.test.mjs` and are outside
  `src/`: two pre-existing in `an-owner-leaves-their-club` and one in `docs/reference/schema.md`
  (the `data` agent's concurrent, in-progress edit in this same checkout). Left unchecked because
  the suite is not green end to end, not because of anything under this task.
- [ ] 8.6 `npm run walk` reaches `/clubs/detail`; confirm it still renders.
- [ ] 8.7 `npm run docs:check` after §9.

## 9. Documentation

- [x] 9.1 `docs/reference/schema.md` — two new rows, each with its audience predicate, its cascade
