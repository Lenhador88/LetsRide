...
- [x] 0.2 **Read `design.md` §D1 before writing a line.** The Narrow reading is a *stated
...
- [x] 0.3 Re-derive the two call sites rather than trusting the line numbers here — a partial fix
  leaves the halves disagreeing: `grep -rn "wouldStrand\|nobody but you" src/`. Measured
  2026-09-03: six hits across `EditRideForm.tsx` and `rides.ts`, **one** of them a comment
  (`EditRideForm.tsx:295`). The other five are all live: the assignment, the render guard, the
  rendered JSX sentence, the `disabled` prop and the action's error string. A first draft said
  "two of them comments" — the rendered alert text reads like prose and is not one, which is the
  §comment-trap error in the direction that *under*-counts the work.
- [x] 0.4 Confirm the premise is still live: the fourth `rides` SELECT arm
...
- [x] 1.1 Add a plain copy module exporting **two** constants: the audience hint and the refusal.
  The module's name is the build's to choose (suggested `src/components/rides/audienceCopy.ts`);
  the strings are not — they are written out in `design.md` §D4.
- [x] 1.2 The hint constant's value is **byte-identical** to what both forms render today:
...
- [x] 1.3 The refusal constant is the new sentence — `design.md` §D4. It replaces **both** the
  form's alert text and the action's error string; there is one copy after this change.
- [x] 1.4 Render the hint constant from `CreateRideForm.tsx` and `EditRideForm.tsx`. **The
  rendered output must not change** — verify by diffing the two rendered strings, not by reading.

## 2. The predicate

- [x] 2.1 Add the pure helper `design.md` §D3 describes — stored pair vs submitted pair, true iff
...
- [x] 2.2 `EditRideForm`: replace `wouldStrand` (`:185`) with the transition predicate computed
...
- [x] 2.3 Gate the `role="alert"` (`:306`) on the new predicate, and render the refusal constant
  in it. **Keep `role="alert"`** — the walk depends on the alert/status split (`design.md` §D5).
- [x] 2.4 Gate the Save button's `disabled` (`:319`) on the same predicate. A ride that arrived
  clubless-and-private must render an enabled Save with no alert.
- [x] 2.5 Rewrite the comment block at `:285-299`. It currently states that the alert argues from
...
- [x] 2.6 `updateRide`: add `is_public, club_id` to the existing `previous` select (`:445`), and
  **move the guard** (`:425-430`) below that read so it can compare. Zod parsing stays first.
- [x] 2.7 The action's prior shape comes from that read and **never from a form field**. When
  `previous` is null, do not refuse and do not permit — fall through and let RLS report.
- [x] 2.8 `createRide` is **not** touched. Confirm by diff; the spec now states the asymmetry is
  deliberate.
...
- [x] 3.1 New `src/components/rides/__tests__/EditRideForm.test.tsx`, **split in two, and the
  split is the plan being wrong rather than the build wandering.** The last three bullets below
  are unreachable from `renderToStaticMarkup`: `EditRideForm` seeds its controlled state FROM the
  ride, so on mount the submitted pair always equals the stored pair and the predicate is false
  for every ride there is. A static render can never reach the disabled state — it takes a rider
  event. So the refused half moved to `EditRideForm.dom.test.tsx` (jsdom, real clicks), which is
  `CLAUDE.md`'s stated bar for reaching for jsdom at all, and it is the file that would catch a
  build hardcoding `disabled={false}`.
  ...
  - **plus two cases the plan did not ask for**: a rider in NO clubs, and `clubs === null`, which
    are the two states where every other control on the form is unavailable;
  - a ride with a club, detached to clubless-and-private, renders the alert and a disabled Save
    (`.dom`);
  - a clubless **public** ride, un-published, renders the alert and a disabled Save (`.dom`);
  - both remedies clear it again — re-ticking public, and picking a club (`.dom`);
  - the alert text is the shared constant, read off `[role="alert"]` (pins §D5's contract).
- [x] 3.2 The hint-pair assertion PD-320's review deferred to this issue: both ride forms render
...
- [x] 3.3 A unit test for the predicate itself, table-driven over the four stored shapes × the
...
- [x] 3.4 An action-level test that `updateRide` refuses the transition against a **mocked
...
- [x] 3.5 `npx vitest run src/components/rides src/lib/actions` green, then `npm run test:unit`.
- [x] 3.6 **Verify each new assertion both ways** (`CLAUDE.md` §Working Principles): break the
...
- [x] 4.1 Read `design.md` §D5's table. **Narrow does not fix PD-311** — a clubless *public*
...
- [x] 4.2 Leave `checkEditRetention`'s `role="status"`-only rule exactly as it is, and do not
...
- [x] 4.5 Read `design.md` §D5b before assuming this change is inert for the tile pipeline. It
  adds no code there, but it routes a new population of rides — every clubless private one —
  through `clear_ride_map_tiles` for the first time. Nothing to build; check PD-385 has the
  pointer.
