# rider-position-question (delta)

> **This is a NEW capability, and every requirement below is ADDED.** PD-170 and PD-419 built the
> whole of this machinery without an OpenSpec change, so `openspec/specs/` holds nothing about the
> location ask — verified rather than assumed:
>
> ```bash
...
> row that draws only in some states — and they now hold **opposite** rules about automatic asks, so
> one capability would carry a contradiction in its own scenarios. The name says `question` rather
> than `priming` because after this change the control primarily asks about a **town**, and only
> secondarily offers a permission.
