...
> **Coordination.** `moderation-digest-delivery` does **not** exist in `openspec/specs/` — this
> change creates it, so every requirement below is ADDED. It is the app's first outbound mail rail
> and it deliberately does not extend `notifications` (in-app rows for riders) or `push-delivery`
> (the active `deliver-push-notifications` change): those two deliver *to riders* and re-check a
> rider's visibility on every send, while this one delivers *to the operator* and has no viewer to
> check. Re-derive with `ls openspec/specs/` and `ls openspec/changes/`.
