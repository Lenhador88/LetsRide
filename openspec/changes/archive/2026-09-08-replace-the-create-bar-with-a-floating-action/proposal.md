...
| **Stale** | The rider's answer changed **in this tab** → the composition follows the answer the write returned, not the read: `setRideAttendance` invalidates without awaiting, so the page holds the answer for the round trip and the chip, the bar and the control flip together and immediately. **Changed in ANOTHER session** → that write is masked for the life of this mount, because the held answer is never cleared. All three follow the held value, the control included, so `the control appears exactly when the write would succeed` is weakened in that one case. Reaching it needs a second session writing this rider's own `ride_members` row *and* something triggering a `rides.*` refetch in this tab during the same mount; `102` limits that write to the rider themselves, and `088`/`111` leave `ride_members` alone on club removal. In the gap RLS still refuses. Stated so it is not rediscovered as a bug. |
