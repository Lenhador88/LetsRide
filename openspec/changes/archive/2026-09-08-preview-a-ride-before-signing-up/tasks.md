...
- [x] 0.1 Read `CLAUDE.md`, `openspec/config.yaml`, and the base capability at
      ...
      already satisfied.
- [x] 0.2 **Settle the migration number.** Re-derive from `ls supabase/migrations/*.sql | tail -3`
...
- [x] 0.3 Record the **before** numbers — every claim in group 6 is a delta against them:
...
- [x] 0.4 Record the **before** pins the migration must not disturb:
...
- [x] 0.5 Re-read `private.live_ride_invite_link`, `private.ride_invite_link_reachable_by` and
...
- [x] 0.6 **Read `public.ride_invite_link_preview`'s return signature off the live database and pin
...
- [x] 0.7 Confirm `public.ride_invite_link_public_preview` does not exist on either project.

## 1. The migration — one function, one grant

- [x] 1.1 Create `public.ride_invite_link_public_preview(t text)` returning
...
- [x] 1.2 `language sql`, `security definer`, `set search_path = ''`, and **`volatile`** — declared
...
- [x] 1.3 Body: `from private.live_ride_invite_link(t) k join public.rides r on r.id = k.ride_id
      ...
      taken from `profiles`.**
- [x] 1.4 `revoke all on function public.ride_invite_link_public_preview(text) from public,
      authenticated;` then `grant execute … to anon;` — **in that order**, and `authenticated` named
      explicitly rather than relied on as absent.
- [x] 1.5 `comment on function` covering: the closed column list; **that it is a strict subset of
...
- [x] 1.6 File header: additive in every statement, **migration-first with the reasoning** (D9's
...
- [x] 1.7 Apply to DEV with `apply_migration`. Record the ordering in
...
- [x] 2.1 **Grant, per grantee.** `has_function_privilege('anon', …, 'EXECUTE')` is `true`;
      `authenticated` and `public` are `false`. Three separate assertions.
- [x] 2.2 **The other three RPCs are unchanged**: `anon` holds no EXECUTE on
      `ride_invite_link_preview`, `claim_ride_invite_link` or `revoke_ride_invite_link`.
- [x] 2.3 **No table became readable**: `has_table_privilege('anon', t, 'SELECT')` is `false` for
      `rides`, `ride_invite_links`, `ride_members`, `ride_invites`, `profiles`, `clubs`.
- [x] 2.4 **The pins from 0.4 still hold** — `can_read_ride`'s `prosrc` md5 and the `rides` SELECT
      qual, byte-identical. A failure here means the migration is wrong, not that the pin is stale.
- [x] 2.5 **No policy names `anon`**: count rows in `pg_policies` for schema `public` whose `roles`
      includes `anon` — zero, before and after.
- [x] 2.6 **The happy path, as `anon`**: a live token returns exactly one row with exactly the closed
      column list.
- [x] 2.7 **The meeting point is returned, and it is the ride's own** — assert the returned value
...
- [x] 2.8 **No crew count, no avatar and no roster**: create a ride with three crew and assert no
      ...
      forward them.
- [x] 2.8b **The projection is a strict subset of `091`'s**, asserted from the catalogue rather than
      ...
      eight is added.
- [x] 2.9 **The six dead states, as `anon`, all six**: revoked, expired, ride deleted, ride departed,
...
- [x] 2.10 **Both previews agree** about each of the six — the anonymous one as `anon`, the
      authenticated one as `authenticated`.
- [x] 2.11 **Club-private is served and unobservable**: a private club's ride and a clubless public
      ride return the identical column shape as `anon`, and neither response carries a club id, club
      name or `is_public`.
- [x] 2.12 **A past ride needs no special case**: a link whose ride departed returns zero rows through
      the ordinary dead path.
- [x] 2.13 **Catalogue reads, never inferred from a call that worked**: `prosecdef` is `true`,
      `proconfig` is `{"search_path="}`, `provolatile` is `v`, and the return signature matches the
      closed list exactly.
- [x] 2.14 **`prosrc` carries no caller predicate**: `is_blocked`, `auth.uid`, `terms_accepted_at`,
      `onboarding_completed_at` all absent. **And no liveness restatement**: `revoked_at`,
      `expires_at`, `departure_at` all absent.
- [x] 2.15 **The anonymous path writes nothing**: preview a live and a dead token as `anon` inside a
      transaction and assert no row was added to any table in `public`.
- [x] 2.16 **Blocked rider, both states**: signed out, **all five fields are returned including the
...
- [x] 2.17 **`anon` reaching further is refused**: selecting the ride from `public.rides` by the id
      the preview returned gives zero rows.
- [x] 2.17b **The grant follows the token and never `is_public`**: with a **public** ride
      ...
      before this change.
- [x] 2.18 Run `PGPASSWORD=postgres npm test`. Reconcile against the previous run by **label set**,
      never by count.

## 3. Security advisors

- [x] 3.1 `get_advisors(security)` on DEV, delta against 0.3. **Expected +1 WARN**, but the label is
...
- [x] 3.2 **If no advisor fires, write that down as a finding**, not as a saving — it means the
      advisor set cannot see the app's only anonymous surface.
- [x] 3.3 **+0 INFO expected**: no table is created, so no `rls_enabled_no_policy`.
- [x] 3.4 Participation gate count unchanged from 0.3. No table, no `authenticated` writer, no gate
      trigger.
- [x] 3.5 The `service_role` census (`CLAUDE.md` §Supabase Rules) is unchanged — 30 kept, 3 revoked.
      Re-run rather than assume.

## 4. The read

- [x] 4.1 New exported function in `src/lib/data/ride-invite-links.ts`, beside
...
- [x] 4.2 **It SHALL NOT call `supabase.auth.getUser()` and SHALL NOT require a session** — that is
      the whole point. `getRideInviteLinkPreview`'s `if (!user) return null` stays where it is.
- [x] 4.3 **No second read.** No `is_crew` probe (there is no caller), no avatar signing (`anon` has
      no reach into `storage.objects`), no crew count.
- [x] 4.4 Zero rows → `null`, meaning *decided: this link is no longer valid*. A thrown error stays a
      thrown error, so the screen can tell a dead link from a tunnel.
- [x] 4.5 A doc block stating what the function does **not** return and why, naming the subset
      relation — the next reader's first instinct will be to forward `crew_count` because `091`
      returns it.

## 5. Types, keys and the screen

- [x] 5.1 New type in `src/types/index.ts`. **Not `Partial<RideInviteLinkPreview>` and not a widened
...
- [x] 5.2 New key in `src/lib/query/keys.ts`, **distinct** from the authenticated preview's. No inline
      key.
- [x] 5.3 `RideInviteJoin` renders the signed-out state: title, start time, meeting point, organiser,
...
- [x] 5.4 The time renders through a named `formatRide*` helper with `rides.timezone` passed as the
...
- [x] 5.5 Gate on the **data**, never on `isLoading`. `undefined` is the skeleton; `null` is the dead
      message.
- [x] 5.6 All eight states from the spec's table render, including offline (cached preview, control
      disabled, nothing queued) and error (retry, distinct from dead).
- [x] 5.7 The CTA routes to signup with the token still stashed — the existing `pending-token` round
      trip, **unchanged**.
- [x] 5.8 **Correct the `RIDE_JOIN_PATH` comment in `src/lib/auth/guard.ts`.** *"public so it can HOLD
...
- [x] 5.9 Buttons are near-black `Grey/100` `#1A1A1A`, never green. Icons from
      `@/components/icons/generated`.
- [x] 5.10 **`noindex, nofollow` on `/rides/join`.** The authenticated wall used to block crawlers by
...
- [x] 5.11 **Do not build from the archived Figma frame.** `npm run figma -- ls "Join ride"` returns
...
- [x] 6.1 Unit test the new read: zero rows → `null`; a thrown error propagates; a malformed token is
      refused before the round trip.
- [x] 6.2 A component test pinning that the signed-out card renders the title **and the meeting
      ...
      the count.
- [x] 6.3 `guard.test.ts` — the four existing `/rides/join` cases still pass unchanged. Behaviour did
      not move; assert that it did not.
- [x] 6.4 **One new walk phase**: open `/rides/join?token=…` **signed out** with `WALK_FIXTURES=1`,
...
- [x] 6.4b **The walk asserts the robots directive** on the same signed-out load: the document
      carries `noindex, nofollow`. It is one line and it is the only automated place 5.10 can be
      caught going missing.
- [x] 6.5 `npx tsc --noEmit`, `npm run lint`, `npm run test:unit`, `npm run build`.
- [x] 6.6 `npm run docs:check` — the full sweep locally, not CI's `--cheap` step.

## 7. Documentation — the main thread writes all of these

- [x] 7.1 **`CLAUDE.md` decision #1**, replaced with the exact wording in `proposal.md` §*This breaks
...
- [x] 7.1b **`CLAUDE.md` decision #2**, replaced with the wording in the same proposal section.
...
- [x] 7.1c **`openspec/config.yaml` `rules.proposal`** — the third rule reads *"A signed-out visitor
...
- [x] 7.2 `CLAUDE.md` advisor accounting — the delta measured in 3.1.
- [x] 7.3 `docs/reference/schema.md` — a `ride_invite_link_public_preview` entry beside the other
...
- [x] 7.4 `docs/reference/migrations.md` §Applied state — the new file's ordering and its
      migration-first reasoning.
- [x] 7.5 **The PR body carries the reversal and the accepted cost**, because neither belongs in the
...
- [x] 8.1 `reviewer` on the final diff. Its findings go on the PR **verbatim**, under `## Review`.
- [x] 8.2 `/opsx:archive` this change before the PR, or say in the PR body why it stays open.
- [x] 8.3 PR against `development`, never `main`.
