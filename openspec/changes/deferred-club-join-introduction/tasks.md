...
- [x] 0.1 Read `proposal.md` §⚠ Read this first and `design.md` §D1 and §D2 before writing any
      code. A1–A4 are the four things in this change that cannot be re-derived from the codebase.
- [x] 0.2 Confirm the forced order still holds — the introduction RPC still refuses a non-member:
...
- [x] 0.3 Confirm the state rule is untouched by anything since the proposal:
      ...
      edits it.
- [x] 0.4 Confirm the private-club routing still holds, so §The negative cases case 2 stays
...
- [x] 1.1 Add a composite write to `src/lib/actions/club-introductions.ts` that joins and then
...
- [x] 1.2 Its result SHALL tell the caller **which** write failed, and SHALL report a successful
...
- [x] 1.3 Each write's existing cache claims stay with that write, issued only on its own success.
      Add no new key.
- [x] 1.4 Header comment: why the order is forced, why there is no compensating delete, and why
      this is not an RPC. One paragraph, pointing at `design.md` §D1 rather than restating it.
- [x] 1.5 Unit test it against the mocked resolver, in `src/lib/actions/__tests__/`, covering all
...
- [x] 2.1 Give `IntroductionPromptBody` a **required** mode — no default. Pre-join draws the
...
- [x] 2.2 The mode latches: pre-join becomes member the moment its own membership write succeeds,
      and never returns. It SHALL NOT be derived from a cache read — `design.md` §D3.
- [x] 2.2a The latch lives **in the sheet, per instance**, and is not hoisted to the page. Explore
...
- [x] 2.2b `onDismiss` gains an argument: whether a membership exists at that instant. The sheet is
      the only thing that knows, and the page is what records the dismissal.
- [x] 2.3 Pre-join `Post` calls the composite from §1; member `Post` keeps calling
      `introduceToClub` unchanged.
- [x] 2.4 The three outcomes render as `client-render-shell`'s table says. The half-succeeded case
      gets the one new string, and the second control relabels with the latch.
- [x] 2.5 Dismissal is inert in pre-join mode from `Post` **until the membership write resolves**,
      ...
      components.
- [x] 2.6 Copy constants go in `src/lib/validation/clubs.ts` beside the existing starter, and
      nothing anywhere compares against any of them.
- [x] 2.7 Both pinned invariants survive in **both** modes: `Post` inert until non-whitespace text,
      and the starter a `placeholder` rather than a `defaultValue`.

## 3. The two join controls

- [x] 3.1 `ClubMembershipButton` takes the club's default-club status and an opener callback. Where
      an introduction is owed it opens the sheet and writes nothing; on the default club it joins
      outright as today.
- [x] 3.2 `JoinClubButton` does the same. **Keep its `hasIntroducedClub` read and move it in front
      ...
      *a join happened*. Rename it if that reads better; do not leave the old name with a new
      meaning.
- [x] 3.2a `ClubMembershipButton` takes the same read on the same terms. Both controls sit above
      cached data and neither may infer freshness from its own position.
- [x] 3.2b Nothing in this change may report a join from a non-erroring upsert alone. Where a
      confirmation is needed that a row was created, read it; otherwise say nothing about it.
- [x] 3.3 Neither control hardcodes the default-club status. `JoinClubButton`'s header already
      records the defect that came of asserting it; keep that reasoning.

## 4. The two screens

- [x] 4.1 `src/app/(app)/clubs/detail/page.tsx`: one sheet, two openers. The state rule is
...
- [x] 4.1a **This screen's own `onDismiss` must stop recording a dismissal for a declined join.**
      ...
      the story is about.
- [x] 4.2 `src/app/(app)/clubs/explore/page.tsx`: the sheet stays owned by the page. Its
      ...
      wrong (`design.md` §D2).
- [x] 4.3 Keep the property that a draft can never be posted to a club other than the one it was
      composed for, however the queue ends up shaped.

## 5. Tests

- [x] 5.1 `IntroductionPrompt.test.tsx`: the four existing **invariants** survive — they describe
...
- [x] 5.2 `JoinClubButton.test.tsx`: tapping Join for a club owing an introduction writes no
...
- [x] 5.2a One test for the dismissal iff on **each** of the two screens, or one test per call site
...
- [x] 5.3 Verify each new test both ways, per `CLAUDE.md`'s verify-both-ways rule — swap the shape
      it pins and confirm it goes red. A test that passes against both shapes pins nothing.
- [x] 5.4 `npx tsc --noEmit`, `npm run lint`, `npm run test:unit`.

## 6. Documentation

- [x] 6.1 `docs/FIGMA-FIDELITY-TODO.md`: the sheet has no v2 frame and now has a second mode. Log
      it where the first one is logged, rather than opening a new section.
- [x] 6.2 No claim added anywhere without the command that checks it, per `CLAUDE.md`.

## 7. The walk

- [x] 7.1 The walk refuses a create and an edit; it does not join a club, and this change does not
