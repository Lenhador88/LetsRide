...
- [ ] 0.1 Read **PD-348**, body **and** comments. Read 2026-08-31: status `Todo AI`, parent PD-326,
  **zero comments**, and the body covers the **admin arm only** — the reporting half is the product
  ...
  overtakes the body.
- [x] 0.2 Confirm the function about to be replaced is still `082`'s. If it has moved, every
  decision in `design.md` D1–D4 is reopened:
  ```sql
  select pg_get_functiondef('public.moderate_club_thread(uuid)'::regprocedure);
  ```
- [x] 0.3 Re-derive the two parent policies. `club_threads` SELECT is what the report INSERT
...
- [x] 0.4 Record the gate-trigger count **before** the migration, so the after-count means
  ...
  ```
- [x] 0.5 Pin `private.is_club_admin_for`'s body **by equality, never by `like`** (`085.28`'s rule
  ...
  ```
- [x] 0.6 Record the advisor baseline. `get_advisors(security)` is the authority; this proxy is what
  ...
  ```
- [x] 0.7 Read the **live** `comment on function public.enforce_participation_gate()` rather than
  ...
  ```
- [x] 0.8 Read the cascade chain off `pg_constraint` rather than remembering it — `076`'s header
  ...
  ```
- [x] 0.9 Read the design offline. **Do not call the Figma API.** `npm run figma -- ls "*eport*"`
...
- [x] 0.10 **Answer `design.md` Q1** before writing §2's policies.

## 1. `094` §1 — the admin arm

- [x] 1.1 `create or replace function public.moderate_club_thread(thread uuid)` — **replace, never
  drop-and-recreate** (`design.md` D3). Signature unchanged, so `create or replace` is available and
  the ACL survives.
- [x] 1.2 Body: `082`'s, with the `clubs` join removed and the whole access-control decision
...
- [x] 1.3 **Exactly ONE raise site**, and no session guard ahead of it (`design.md` D4):
  ...
  that id" src/`.
- [x] 1.4 **Do not** re-issue `revoke`/`grant` on the function. `create or replace` keeps the ACL,
  and re-issuing them here would make a future reader think the ACL had been reset.
- [x] 1.5 `comment on function public.moderate_club_thread(uuid)` — restate the widened authority,
...
- [x] 2.1 The table, on `011` §4's shape: `id uuid default uuid_generate_v4() primary key`,
...
- [x] 2.2 Three constraints, `011`'s verbatim with the subject renamed: `reason in ('spam',
...
- [x] 2.3 `create index … on public.club_thread_reports (thread_id)` — for the `club_threads`
  cascade and the queue's join. `029`'s standing rule, and neither the PK nor the unique index
  serves it.
- [x] 2.4 `alter table … enable row level security`.
- [x] 2.5 `comment on table` stating: the audience predicate it inherits and from where; that
...
- [x] 2.6 `comment on column … created_at` — server-owned, withheld from the INSERT column grant,
...
- [x] 3.1 SELECT: `using (reporter_id = auth.uid())`. **No membership conjunct** — N28, and write
  the comment that says why, or the next reader adds one for consistency.
- [x] 3.2 INSERT: `with check (reporter_id = auth.uid() and exists (select 1 from
...
- [x] 3.3 **No UPDATE and no DELETE policy.** Absence is the enforcement; assert it in both
  directions in §5, because a well-meaning `grant all` restores only one of them.
- [x] 3.4 `revoke all on public.club_thread_reports from public, anon, authenticated,
...
- [x] 3.5 `grant select on public.club_thread_reports to authenticated;` and `grant insert
...
- [x] 4.1 `drop trigger if exists enforce_participation_gate on public.club_thread_reports;` then
...
- [x] 4.2 Update `comment on function public.enforce_participation_gate()` — composed from the
...
- [x] 5.1 A club **admin** (a `club_members` row with `role='admin'`) calls `moderate_club_thread`
...
- [x] 5.2 The club **owner** still succeeds. Fails against a body that gates on `club_members.role`
  alone in the one case §5.3 names, and must be asserted anyway as the no-regression baseline.
- [x] 5.3 **An owner holding NO `club_members` row still succeeds.** The `054`/PD-128 state, built
...
- [x] 5.4 A **plain member** is refused — both the RPC and a direct `delete from club_threads`.
- [x] 5.5 A **non-member** is refused, for a public club and for a private one.
- [x] 5.6 An **admin of a different club** is refused on this club's thread. Proves the club
  argument is not ignored.
- [x] 5.7 The **author who is a plain member** is refused *by the RPC* while their own policy delete
  still succeeds. Two assertions, not one.
- [x] 5.8 **The refusal is indistinguishable from "no such thread"**: call the RPC with a random
...
- [x] 5.9 **A block does not take the right away, in either direction**: an admin who has blocked the
...
- [x] 5.10 `has_function_privilege('anon','public.moderate_club_thread(uuid)','execute')` is
  **false** and `('authenticated', …)` is **true**, after the migration. The drop-and-recreate
  tripwire (`design.md` D3).

**Reporting:**

- [x] 5.11 A club **member** reports a thread: succeeds, one row, `reporter_id` = them.
- [x] 5.12 A **non-member** is refused — **public** club and private club, both. The public case is
  the one the `EXISTS` is carrying alone.
- [x] 5.13 A rider **blocked with the author** (each direction) is refused. N14; assert it as a
  *designed* consequence with a comment, so a later session does not "fix" it.
- [x] 5.14 A rider cannot report **as somebody else** — `reporter_id` set to another uuid is refused
  `42501`.
- [x] 5.15 A **second report of the same thread by the same rider** is refused by
  `club_thread_reports_one_per_rider` (`23505`), and the same rider reporting a **different** thread
  succeeds.
- [x] 5.16 The **author of the thread reads zero** reports about it. The load-bearing one (N15);
  `rls_test.sql`'s postcard twin is the model.
- [x] 5.17 The **club owner reads zero** and the **club admin reads zero**. Two assertions, and
  neither is implied by §5.16.
- [x] 5.18 The **reporter reads their own row**, still reads it **after leaving the club**, and
  still reads it **after blocking the author**. Three assertions, N28 and N29.
- [x] 5.19 The `reason` CHECK accepts exactly the six values and refuses a seventh — read the
...
- [x] 5.20 `created_at` is **not** in the INSERT column grant: a client naming it is refused
...
- [x] 5.21 **No UPDATE and no DELETE**, policy *and* grant, for `authenticated`. Read the sorted
...
- [x] 5.22 `anon` holds **no** privilege on the table and **no policy targets a role other than
  `authenticated`**.
- [x] 5.23 `has_table_privilege('service_role','public.club_thread_reports','select')` is **false**
...
- [x] 5.24 **Cascades**: deleting the thread removes its reports; deleting the club removes them
  through the thread; deleting the reporter's profile removes them.
- [x] 5.25 Every FK into `profiles` on the new table leads an index — the `pg_constraint`/`pg_index`
  form `029` uses, never a timing.
- [x] 5.26 `enforce_participation_gate` is present **by table name** on `club_thread_reports`, and
  ...
  `23514`.
- [x] 5.27 `private` reachability, all four false: `has_schema_privilege('authenticated','private',
...
- [x] 5.28 Re-run the whole suite and **compare label sets, not counts** — a count cannot tell a
  rename from a loss, which is what `038` did to one of `036`'s assertions.

## 6. `094` §5 — the reader, and the client

- [x] 6.1 `private.club_thread_report_queue`, `with (security_invoker = false)` **written out** —
...
- [x] 6.2 `private.remove_reported_thread(target uuid) returns jsonb` — **not** `security definer`,
...
- [x] 6.3 `revoke all on private.club_thread_report_queue from public, anon, authenticated,
...
- [x] 6.4 A `§Operating it` footer in the migration, `076`'s: what to run, in what order, and the
...
- [x] 8.1 `PGPASSWORD=postgres npm test` — the RLS suite, green, with §5 in it.
...
- [x] 9.1 `docs/reference/schema.md` — a new `club_thread_reports` row carrying its audience
...
- [x] 9.4 Do **not** edit `CLAUDE.md` or `docs/HANDOFF.md` from an agent; the main thread owns both.
