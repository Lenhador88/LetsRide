...
- [ ] 0.4 **BLOCKING task 1 — re-derive the migration number.** `CLAUDE.md` says 60 files with DEV at
...
- [ ] 0.5 **BLOCKING task 3 — record the advisor count BEFORE applying.** This change must move it by
...
- [ ] 1.1 **The table.** `user_id uuid references public.profiles(id) on delete cascade not null`,
...
- [ ] 1.2 **`alter table … enable row level security`**, and a table comment stating the audience in
...
- [ ] 1.3 **SELECT policy** — `to authenticated`, `using (user_id = auth.uid())` and nothing wider.
...
- [ ] 1.4 **INSERT policy** — `to authenticated`, `with check (user_id = auth.uid() and exists (select
...
- [ ] 1.5 **UPDATE policy** — `using (user_id = auth.uid())`, `with check` identical to 1.4. Comment
...
- [ ] 1.6 **No DELETE policy and no DELETE grant**, with the **honest** reason.
...
- [ ] 1.7 **The `last_read_at` trigger** — `before insert or update`, setting `new.last_read_at :=
...
- [ ] 1.8 **NO participation-gate trigger**, and say so in the file. `023`'s own header excludes
...
- [ ] 1.9 **`public.ride_has_unread(ride uuid) returns boolean`** — `language sql`, `stable`,
...
- [ ] 1.10 **Grants.** `revoke all on public.ride_reads from anon, authenticated;` then
...
- [ ] 1.11 **A §Verification footer**, in `015`'s shape, each line predicting a number. **Every grant
...
- [ ] 2.1 **The write predicate, one assertion per conjunct**, so that removing either fails a
...
- [ ] 2.2 **The existence oracle is closed.** A rider not on the crew writing a watermark for (a) a
...
- [ ] 2.3 **`user_id` cannot be forged**, on INSERT and on the UPDATE arm's `USING` — a rider must not
  be able to move a row out of their own ownership.

- [ ] 2.4 **Nobody reads another rider's watermark.** Assert it for a crew member **and name the
  organizer specifically**, because the organizer is the role a "who has seen this" affordance would
  be built for.

- [ ] 2.5 **`ride_has_unread` under RLS**, as each role in `specs/ride-chat-unread/spec.md`'s per-role
  table:
...
- [ ] 2.6 **The grants, scoped to the grantee.** `has_table_privilege('authenticated',
...
- [ ] 2.7 **The trigger imposes the timestamp on both arms.** Insert naming a far-future
...
- [ ] 2.8 **Cascades.** Deleting the ride removes the watermark; deleting the rider removes it;
...
- [ ] 2.9 **`anon` reaches nothing** — zero grants on the table, no EXECUTE on the reader.

- [ ] 2.10 Run the suite, and reconcile by label set:
  ```bash
...
- [ ] 3.1 **Apply `061` to DEV** (`fpmrimzxadewsaiwpsel`) with `apply_migration`. The file is small
  enough to pass as a string, so none of the reduction technique for large migrations applies.
- [ ] 3.2 **Run every line of the §Verification footer against DEV.** The suite runs on plain
  Postgres and cannot see role grants, exposed RPC endpoints or Supabase defaults — the gap `031`
  exists because of.
- [ ] 3.3 **Advisors: expect the same number as 0.5, unchanged.** A new WARN means either the reader
  was written `security definer` or the trigger function's `revoke` did not land.
- [ ] 3.4 **Drift check.** `npm run db:drift` compares *names*, never versions or ordering. DEV ahead
  of PROD is the ordinary state between merge and promotion, not drift.
- [ ] 3.5 **PROD is a separate, later step** and rides the promotion to `main`. Do not apply to PROD
  from a feature branch.

## 4. The data layer and the action — `src/lib/data/`, `src/lib/actions/`

- [ ] 4.1 **`getRideChatUnread(rideId)`** in `src/lib/data/ride-messages.ts` — one `supabase.rpc`
...
- [ ] 4.2 **`markRideChatSeen(rideId)`** in `src/lib/actions/ride-messages.ts` — resolve the client,
...
- [ ] 4.3 **`invalidate(queryKeys.rides.unread(rideId))` and nothing else** at the foot of
...
- [ ] 4.4 **Verify the index choice on DEV** with a real thread rather than trusting the proposal:
  `explain (analyze, buffers)` on the `exists` for a ride with a few hundred messages. Expect an index
  scan on `ride_messages_ride_id_idx (ride_id, created_at, id)`. **This is the one performance claim
  in these artifacts that was written with no database access** — if it comes back a sequential scan,
  say so rather than adding an index quietly, because the index that would fix it belongs to `034`.

- [ ] 4.5 **`sendRideMessage` does NOT change.** If a diff touches it, the key nesting in §5 is wrong.

## 5. The cache key — `src/lib/query/keys.ts`

- [ ] 5.1 **One key, nested under the thread's:**
  `unread: (rideId: string): QueryKey => ['rides', 'detail', rideId, 'messages', 'unread']`.

- [ ] 5.2 **Update the `rides.messages` docstring**, which currently predicts this change by name:
...
- [ ] 5.3 **Document the asymmetry beside the new key**, because the file already records a nesting
...
- [ ] 5.4 `npx vitest run src/lib/query/__tests__/keys.test.ts` — that file asserts every
  `invalidate()` argument comes from `keys.ts` rather than being spelled inline.

## 6. The components — last, and both of them small

- [ ] 6.1 **`src/components/rides/MarkRideChatSeen.tsx`** — `'use client'`, renders `null`, calls
...
- [ ] 6.2 **Mount it in `src/app/(app)/rides/detail/chat/page.tsx`, conditionally on `isCrew`.**
...
- [ ] 6.3 **`src/components/rides/RideChatUnreadDot.tsx`** — `'use client'`, one
...
- [ ] 6.4 **Mount it inside `RideHeader`'s existing `action` slot**, under the condition that slot
...
- [ ] 6.5 **Update `RideHeader`'s docstring.** It currently says the dot is *"Not drawn — there is no
...
- [ ] 6.6 **Nothing under `src/components/ui/` or `src/components/icons/` is touched.**
  `NotificationDot` is reused unchanged. Verify with the diff, not by intention.

- [ ] 6.7 **No dot on `RidePageMenu`'s `Chat` row** — Q2's default. If Q2 comes back the other way it
...
- [ ] 7.1 `npx tsc --noEmit`
- [ ] 7.2 `npm run lint`
- [ ] 7.3 `npm run test:unit`
- [ ] 7.4 `npm run build` — needs `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`. It
...
- [ ] 7.5 `PGPASSWORD=postgres npm test` — the RLS suite, green, with §2's labels present.
- [ ] 7.6 `npm run docs:check` — the full sweep locally, not the `--cheap` subset CI runs. Any numeric
  claim this change moved (the participation-gate count is *unchanged*, and that is itself a claim)
  must still match.
- [ ] 7.7 **The walk is optional here and says little.** It asks one question per route — did this
...
- [ ] 8.1 **`015` §2's cascade sentence.** *"Leaving a club cascades the row away via the FK"* — the
...
- [ ] 8.2 **`feed_reads.last_seen_at` is written from the device clock** (Q4). Not fixed here; it
  belongs in `docs/HANDOFF.md` §Known issues with the one-line reason, so it is findable.
- [ ] 8.3 **`club_unread_counts()` does not exclude the reader's own postcards**, and a postcard is
  ...
  fix it here.
- [ ] 8.4 **`CLAUDE.md`'s participation-gate list stays at ten.** This change adds a table
...
- [ ] 9.1 **`ride-chat` / `The surfaces this change does not build`** is MODIFIED by this change and
...
- [ ] 9.2 **This change modifies nothing in `client-cache-invalidation` or
...
- [ ] 9.3 **`enforce-creator-membership`** makes 1.9's third coalesce arm redundant and never wrong.
...
- [ ] 9.4 **`grant-club-owner-member-reach` and `align-fanout-recipients-with-readability`** both
...
- [ ] 9.5 **`tag-postcards-to-rides` / `A rider-supplied reference SHALL have its referent's
...
- [ ] 10.1 Move **PD-120** to `Development (AI)` on pickup, not at the end — it is the concurrency
  lock and the only signal saying which story is being worked right now. Pass the project **id**,
  never the name.
- [ ] 10.2 The issue body points at this proposal and does **not** restate it. Its five-rating block
  stays; a Linear issue that grows a specification is a bug.
- [ ] 10.3 **Q2, Q3 and Q5 are the product owner's alone.** All three have defaults, so none blocks
  the build — put them in the PR body as questions rather than moving the issue to `Needs help`,
  which stops every dispatch.
- [ ] 10.4 File Q4 (the `feed_reads` clock) and 8.3 (the own-postcard counter) as their own issues
  with short titles, so neither is inherited as covered by this one.
- [ ] 10.5 Never type a status name from memory — `list_issue_statuses team=Pedro & Dave`. A
...
- [ ] 11.1 **Run `reviewer` on this proposal, before any code** — the first of its two passes and the
...
- [ ] 11.2 **Run `reviewer` again on the final diff**, once, immediately before the PR. It touches
  `src/` and `supabase/`, so the RLS and data-exposure passes both fire.
- [ ] 11.3 Branch off `development`; open the PR against `development`, never `main`.
