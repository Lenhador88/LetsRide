...
- [ ] 0.1 Confirm the territory matches PD-447's marker: `src/lib/location/`,
...
- [ ] 0.2 Log the copy as a departure in `docs/FIGMA-FIDELITY-TODO.md`, **replacing** the existing
...
- [ ] 0.3 Re-read `openspec/specs/client-render-shell/spec.md` §*A privacy control SHALL NOT render a
...
- [ ] 1.1 **New file, and delete `src/lib/location/ask-once.ts`.** The name `ask-once` becomes a lie
...
- [ ] 1.2 Key `letsride.location.dismissedQuestion`, value `{"at":<epoch ms>,"n":<int ≥ 0>}` as JSON.
      **`n` counts consecutive dismissals.**
- [ ] 1.3 Export the interval as a **pure** function of the record and a `now` argument, so every
...
- [ ] 1.4 `isQuiet(record, now)` is true iff the record parses, `now >= at`, and `now < at + quietFor(n)`.
      **A record with `at > now` is treated as absent** — a device clock that moved backwards must
      not silence the question for ever.
- [ ] 1.5 Validate the **shape**, not just the parse (D6): a non-null object, `at` a finite number,
      `n` a non-negative integer. Everything else — a throw, `null`, `1`, `"x"`, `[]`, a missing
      field — is *absent*, which fails **open**.
- [ ] 1.6 `recordDismissal(now)` writes `{ at: now, n: previous + 1 }`, reading `previous` through
      the same validated parse so a corrupt record restarts the ladder at 1 rather than at `NaN`.
- [ ] 1.7 `recordAnswered(now)` writes `{ at: now, n: 0 }` — quiet for the base interval with the
      ladder reset. `clearDismissal()` removes the key outright, for the grant and for sign-out.
- [ ] 1.8 **Remove the retired key.** `clearDismissal()` also deletes `letsride.location.asked`, so a
      returning rider's device is not left carrying a value nothing reads. Never derive behaviour
      from its presence.
- [ ] 1.9 Carry forward the two paragraphs of `ask-once.ts`'s header that are still true — why it
...
- [ ] 1.10 Keep a `resetDismissalForTests()` seam matching `resetRiderLocationCacheForTests`.

## 2. The decision — `src/lib/location/priming.ts`

- [ ] 2.1 Widen `LocationPrimingState` to
      `'hidden' | 'ask' | 'blocked' | 'town' | 'refine' | 'confirm'` (D4).
- [ ] 2.2 Add two inputs: `town: string | null | undefined` (**raw** `profiles.location`;
...
- [ ] 2.3 **Lift the exclusion for the town half only.** Profile-sourced position with permission
      `denied` **or** `unavailable` → `confirm`. Profile-sourced with `prompt` stays `refine`.
      Nothing in `confirm` may reach the geolocation API.
- [ ] 2.4 Keep `granted` → `hidden` in all three of its sub-states, and keep device-sourced +
...
- [ ] 2.5 Add the label function beside the state (D3 — **not** in `explore-label.ts` or
      ...
      why (D1).
- [ ] 2.6 Replace the header's PD-419 section with the reversal: what was removed, what was kept, and
      the accepted cost. A later session must not read the missing timer as an oversight.
- [ ] 2.7 Rewrite `src/lib/location/__tests__/priming.test.ts` **exhaustively over the proposal's
...
- [ ] 2.8 **Verify the tests both ways.** Mutate the implementation — return `refine` instead of
...
- [ ] 2.9 New `src/lib/location/__tests__/dismissal.test.ts`: the ladder's four rungs and its cap, a
...
- [ ] 3.1 Rename `UseMyLocationRow` to `LocationQuestionRow` (file and export). The old name
      describes an offer the row no longer leads with.
- [ ] 3.2 **Delete `auto`, `AUTO_ASK_DELAY_MS`, the timer effect and the `riderOpenedSheet` ref —
      with their headers** (D5). The latch has exactly one reader and it is the timer.
- [ ] 3.3 `town` becomes a **required** prop typed `string | null | undefined`, raw. Its docstring
      says `undefined` is unsettled and draws nothing, and that it is the **column**, not
      `nearLabel`'s output.
- [ ] 3.4 Add a `quiet`/dismissal read and hold the post-dismissal state locally too, so the row
      disappears in the same render as the dismissal rather than on the next mount.
- [ ] 3.5 Route by state: `ask`/`refine` → `LocationPrimingSheet` `mode="ask"`; `blocked` →
      `mode="blocked"`; `town`/`confirm` → `TownQuestionSheet` directly. **No sheet copy changes.**
- [ ] 3.6 Record a dismissal on **every** close without an answer — the sheet buttons, the scrim and
      Escape (Q2's default). Do **not** record one when `TownQuestionSheet` reports `onSaved`, and do
      not record one when a fix came back.
- [ ] 3.7 On a fix: keep the existing pair — the resolver memo is overwritten and
      `invalidate(queryKeys.riderLocation())` is called — and add `clearDismissal()`.
- [ ] 3.8 Keep the `refine` path's existing behaviour on a denial: the row re-renders as `confirm`
...
- [ ] 3.9 Keep the accessible name saying what tapping does, per the spec's scenario.
- [ ] 3.10 Rewrite `UseMyLocationRow.dom.test.tsx` as `LocationQuestionRow.dom.test.tsx`: delete the
...
- [ ] 4.1 `src/app/(app)/rides/page.tsx` (~205) — delete the row and its comment block; the strip
      stays. Check whether `label` is still used by the strip (it is) before touching the read.
- [ ] 4.2 `src/app/(app)/clubs/page.tsx` (~171) — same, including the `className="px-0"` wrapper
      argument, which goes with it.
- [ ] 4.3 `src/app/(app)/rides/explore/page.tsx` (~110) — drop `auto`, pass `town={city.data}` raw,
      and **move the row outside the `rides.data` branch** (D7) so it renders during load and on the
      error path.
- [ ] 4.4 `src/app/(app)/clubs/explore/page.tsx` (~120) — drop `auto`, pass `town={city.data}` raw.
...
- [ ] 4.5 Add a render assertion to the two tab roots' tests (or one new component test) that the
...
- [ ] 5.2 `signOut` in `src/lib/actions/auth.ts` calls `clearDismissal()` alongside
...
- [ ] 5.3 Confirm `src/lib/actions/__tests__/writers-invalidate.test.ts` still passes and that
...
- [ ] 6.1 `src/components/push/PushPrimingRow.tsx` — its header says the location row draws on
...
- [ ] 6.2 `src/components/profile/LocationSetting.tsx` — its header names `UseMyLocationRow` as the
      one control that may prompt. Update the name and confirm the sentence stays true.
- [ ] 6.3 `docs/reference/running-locally.md` line ~15 names `ask-once.test.ts` in the Node-26
...
- [ ] 6.4 `scripts/walk.mjs` — delete `dismissLocationSheet()` and its three call sites (~3513,
...
- [ ] 7.2 `npm run docs:check` and `npx vitest run scripts/docs/__tests__/crossrefs.test.mjs` — these
...
- [ ] 7.7 Sign out and back in as the other fixture; confirm the question is asked rather than
      inherited.
- [ ] 7.8 Confirm the diff touches **no `supabase/` file**, so the config's migration/assertion
      pairing is satisfied by there being no migration.

## 8. Record

- [ ] 8.1 Comment on PD-447 with the settled interval, the defaults taken for Q1–Q4, and the two
      findings that contradict its premise: `nearLabel`'s `you` (D1) and `ExploreRidesStrip.test.tsx`
      not needing a rewrite (D2).
- [ ] 8.2 Note on PD-419 that its automatic ask has been deliberately reversed, with the accepted
      cost, so the reversal is visible from the issue that introduced it.
- [ ] 8.3 `reviewer` on the final diff, before the PR opens. It did not write this, which is the
      point.
- [ ] 8.4 `/opsx:archive` this change before the PR, or say in the PR body why it stays open.
- [ ] 8.5 PR to **`development`**, merged in the same session; PD-447 reaches `Deployed to DEV` when
      it is running there.
