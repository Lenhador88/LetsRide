...
- [ ] 1.3b Replace `'onboarding cannot be completed before username and location are set'` with
...
- [ ] 1.9 `create or replace function public.username_exists(p_username text)` — reproduce whole
      ...
      **not** fix.
- [ ] 1.10 Do **not** touch `018`'s `profiles_location_length`, the `location` column, its grants,
      any policy, or `enforce_participation_gate`.
- [ ] 1.11 File header: the three places the invariant lived, why the RPC's copy is the
...
- [ ] 1.12 Footer verification block: the queries that show both functions' new bodies, that the
...
- [ ] 2.9 `username_exists`: the caller's own name — and its case variants — read **available**;
...
- [ ] 2.10 Assert the new refusal message at least once, by text and not only by SQLSTATE, so 1.3b
      cannot silently regress. One assertion is enough; the point is that *some* gate reads the
      string.
- [ ] 2.11 `PGPASSWORD=postgres npm test` green, and reconcile the assertion **label set** against
...
- [ ] 3.9 `src/lib/auth/guard-cache.ts` — **comments only, no code**. Its §Writers header and
...
- [ ] 3.10 `git grep -n "setLocation"` before opening the PR and confirm every remaining hit is
...
- [ ] 3.11 `npx tsc --noEmit`, `npm run lint`, `npm run test:unit` green.
...
- [ ] 3b.1 `src/lib/validation/profile.ts`: `location` takes the `optionalText` shape — trim,
...
- [ ] 3b.2 `optionalText` is declared **below** `locationSchema` today, so the definition has to
      move rather than be edited where it sits — a `const` referenced before initialisation is a
      runtime error, not a type error.
- [ ] 3b.3 Delete `locationSchema`'s `min(1, 'Tell us where you ride from.')`. Check whether
...
- [ ] 3b.4 That schema's doc comment describes `003`'s completion trigger refusing the stamp while
      `location` is NULL. That is what `075` removes — rewrite it, do not leave it.
- [ ] 3b.5 `src/components/profile/EditProfileForm.tsx`: remove `required` from the location Input.
...
- [ ] 3b.6 Confirm by hand on DEV that a rider with NULL `location` can save a bio change, and that
...
- [ ] 6.4 Confirm the ADDED requirements — four under `database-enforced-integrity`, two under
...
- [ ] 7.4 `npm run walk` against DEV, with the relay (`scripts/supabase-relay.mjs`) — read its
      ...
      **add `['/onboarding/location', '/postcards']`** so the deleted route's redirect is asserted
      ...
      existing phase.
- [ ] 7.4b **`checkEditProfileRetention` needs a new refusal trigger before 3b.1 lands, or it goes
...
- [ ] 7.4c Rewrite that phase's header comment. It justifies the first assertion with *"`023`'s
...
- [ ] 7.5 `get_advisors(security)` on DEV after apply. Expect the same ten as the advisors table in
...
- [ ] 8.2 Re-read PD-286's title before writing a status. It names dropping the step; if any part
