...
- [x] 1.1 Confirm `private.is_club_member`'s body is still what `design.md` §D5 records, on
...
- [x] 1.2 **Re-verify the block interaction** (`proposal.md` N4, `design.md` §D4) against
...
- [x] 1.3 Re-derive **both** caller sets rather than copying the proposal's tables — they are the
...
- [x] 1.3a **Confirm the recursion dependency still holds** (`proposal.md` §Impact): `clubs` must
...
- [x] 1.4 Re-run the ownerless-owner census on both projects and record the numbers **in the
...
- [x] 1.5 Confirm the next free migration number with `ls supabase/migrations/` against
...
- [x] 2.1 Write `supabase/migrations/054_club_owner_is_a_member.sql`, a single
...
- [x] 2.2 In the same statement, set `search_path = ''` and schema-qualify every reference
...
- [x] 2.3 Order the two arms so the membership `EXISTS` is evaluated first and the `clubs` lookup
...
- [x] 2.4 Add `COMMENT ON FUNCTION private.is_club_member(uuid)` stating the two-arm contract and
      the reason the name was not changed (`design.md` §D2), so the next reader does not file the
      owner arm as a bug.
- [x] 2.5 Header comment carries: the PD-128 reference, the 1.4 census numbers, the rollback body
...
- [x] 2.6 Do **not** touch `club_members` DELETE, any trigger, any grant, or any policy. The
...
> **Build note — where `assert_denied` was replaced, and why it had to be.** Tasks 3.11c and 3.12
> ask for `assert_denied` on four statements: deleting another rider's `ride_messages` row,
> updating the member's ride, updating the member's postcard, and deleting the member's
> `club_members` row. **`assert_denied` fails against a correct policy for all four.** Each is
> keyed in its policy's `USING` clause, and a `USING` clause *filters* an UPDATE or DELETE to zero
> rows rather than raising `42501` — so the statement returns cleanly and `assert_denied`, which
> requires an `insufficient_privilege` exception, reports `FAIL … expected an RLS denial, but the
> statement succeeded`. This is the same asymmetry `harness.sql` documents at length on
> `assert_allowed`, which now *refuses* to accept an UPDATE or DELETE for it. All four are
> therefore written as the three-line idiom that comment prescribes — `savepoint`, run the
> statement, assert the row is unchanged, `rollback to savepoint` — which proves the write was
> refused rather than merely un-raised. The `assert_denied` calls that remain are all INSERTs,
> where a `WITH CHECK` violation genuinely does raise.

- [x] 3.1 **Fixture.** A private club whose `owner_id` holds **no** `club_members` row, a second
      ...
      real route in.
- [x] 3.2 **Positive — the reported bug, read.** As the ownerless owner: the club ride is
      returned. Assert the count, not merely that no error was raised.
- [x] 3.3 **Positive — the reported bug, write.** As the ownerless owner: `assert_allowed` an
      insert of a ride carrying that `club_id`. This is the half the issue notes is easy to miss.
- [x] 3.4 **Positive — the widened set.** As the ownerless owner: the club's roster is readable,
      a postcard into the club is insertable and readable, and a `feed_reads` row for the club is
      insertable.
- [x] 3.5 **N1.** As the third rider (neither owner nor member): zero rides, zero postcards, zero
      roster rows for the private club.
- [x] 3.6 **N2.** As a rider who owns **no** club and has just deleted their `club_members` row:
      zero rides immediately. Proves the arm keys on `clubs.owner_id`, not on history.
- [x] 3.7 **N4 — the block, and this is the assertion that matters most.** With a block row
...
- [x] 3.8 **N4, continued.** A blocked rider's postcard in the owner's own club is not returned to
      the owner; a blocked rider's `club_members` row is absent from the roster the owner reads.
- [x] 3.9 **N5.** The owner of club A reads zero rides, postcards and roster rows for private
      club B.
- [x] 3.10 **N6.** With `auth.uid()` NULL and `set local role anon`, every table in the caller set
...
- [x] 3.11 **N7, part one — ownership alone confers no chat.** As the ownerless owner **holding no
...
- [x] 3.11a **N7, part two — the join path SHALL work, and must be asserted as a positive.** As
...
- [x] 3.11b **N7, part three — the block dominates the join.** With a block between the ownerless
      ...
      than open.
- [x] 3.11c **N8 for chat.** As a crew-joined owner: `assert_denied` on deleting another rider's
      `ride_messages` row in a ride they did not organize.
- [x] 3.11d **Storage — the widening that reaches image bytes.** The RLS suite's harness shims
...
- [x] 3.12 **N8 / N9.** As the ownerless owner: `assert_denied` on updating the member's ride,
      on updating the member's postcard, and on deleting the member's `club_members` row.
- [x] 3.13 **N3.** Assert that **no policy predicate references `admin` at all** — the suite
...
- [x] 3.14 **Regression floor.** Assert a plain member's reach into a private club is unchanged,
      so the migration cannot pass by widening everyone.
- [x] 3.15 Run `PGPASSWORD=postgres npm test` green. **Compare label sets, not counts**, against
      the pre-change run — a count cannot tell a rename from a loss.

## 4. Apply and verify

- [x] 4.1 Apply `054` to DEV (`fpmrimzxadewsaiwpsel`). No hand-exercise gate is needed: this
      change hangs no trigger on a live write path, so no rider's transaction runs new code.
- [x] 4.2 Verify the applied function against the file —
...
- [x] 4.3 Re-run 1.3's caller query and confirm it still returns the same 10 policies, none
      recreated: `select count(*) from pg_policies where …`. A changed count means something
      dropped a policy.
- [x] 4.4 **Check the security advisors** — `get_advisors(security)` on DEV.
...
- [x] 4.5 Exercise it by hand on DEV in a **rolled-back transaction**: impersonate an ownerless
...
- [x] 5.1 Update `docs/reference/schema.md` where it describes the club audience predicate — it
      is the per-table contract and it currently describes a membership-only test.
- [x] 5.2 Add the owner arm to `docs/reference/migrations.md`'s chain notes if `054` acquires any
      ordering relationship. It has none today (`design.md` §D6); record that it has none rather
      than leaving the question open.
- [x] 5.3 Do **not** write `CLAUDE.md` or `docs/HANDOFF.md` from an agent — the main thread owns
      the docs spine.

## 6. Archive

- [x] 6.1 `npx openspec validate grant-club-owner-member-reach --strict`.
- [x] 6.2 Before archiving, re-check that no other change has since modified
...
- [x] 6.3 Do not archive while the RLS suite is failing.
