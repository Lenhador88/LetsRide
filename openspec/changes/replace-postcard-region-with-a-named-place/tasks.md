...
- [ ] 1.1 Confirm the number. `071` is the highest file and no in-flight proposal claims `072` —
...
- [ ] 1.2 **GATE.** Read both absolute lists off the live database and build §1.8/§1.9 from the
...
- [ ] 1.3 Add the two columns: `taken_place_name text null`, `taken_place_id text null`.
      The `taken_` prefix is `064`'s and is load-bearing — where the photo was taken is not where
      the postcard was posted.
- [ ] 1.4 Add `postcards_taken_place_name_length` — `taken_place_name is null or
      char_length(taken_place_name) <= 200`, mirroring `clubs_location_name_length`.
- [ ] 1.5 Add `postcards_taken_place_id_length` — `taken_place_id is null or
...
- [ ] 1.6 Drop and re-add `postcards_taken_location_coupling` with the five arms in
...
- [ ] 1.7 Drop `postcards_region_location_is_rounded` and add
...
- [ ] 1.8 Issue the absolute INSERT grant: §1.2's list plus `taken_place_name, taken_place_id`.
- [ ] 1.9 Issue the absolute SELECT grant: §1.2's list plus the same two.
- [ ] 1.10 **Issue no UPDATE statement of any kind.** Leaving it alone is what produces the
...
- [ ] 1.11 Grant nothing to `anon` — decision #1, and `007` revoked the last of them.
- [ ] 1.12 Write no `UPDATE`, `DELETE` or `INSERT` against `postcards`. There is no backfill;
      `design.md` §D9 is the reasoning and the spec carries it as a requirement.
- [ ] 1.13 Column comments on both new columns, in `064`'s register: what the value is, what it is
...
- [ ] 1.14 File header: what is wrong, why this is a privacy change rather than a schema chore,
...
- [ ] 2.1 `shape.ts`: widen `SearchMode` to include `'reverse'`, and widen `ProxyRequest` to carry
      a coordinate. Keep the rule that **no subject is read from the body** — there is still no
      field for a user id.
- [ ] 2.2 `shape.ts`: range-check the inbound coordinate with the same four comparisons
...
- [ ] 2.3 `shape.ts`: `isSearchable`'s per-mode floor has no meaning for a coordinate. Give the
...
- [ ] 2.4 `shape.ts`: the reverse endpoint URL builder and the response mapper. **Write the
...
- [ ] 2.5 `index.ts`: route the new mode after the ledger insert, alongside the other two. Change
      nothing about the order of operations.
- [ ] 2.6 `src/__tests__/place-search-shape.test.ts`: cases for the new mode's parse, its range
...
- [ ] 2.7 **Replace §2.4's documented type with a measured payload** the first time anyone can
...
- [ ] 2.8 **Deploying is an owner action** — no CLI in the container, and `deploy_edge_function` is
...
- [ ] 3.1 `reversePlace(lat, lon)` — sends the **rounded** coordinate, per
...
- [ ] 3.2 It **degrades to `null` on every failure and throws none of the four typed errors**,
...
- [ ] 3.3 Read the unsupported-mode code off the error body through `edgeFunctionErrorCode` and
...
- [ ] 3.4 A unit test for the latch: two calls, one probe. This is the whole degradation contract
      and it is invisible in any other gate.

## 4. The composer — `src/components/postcards/CreatePostcardForm.tsx`

- [ ] 4.1 Render the `Location` block unconditionally, not behind `upload.status === 'done'`.
      Remove the "This photo has no location." sentence — the rider can now supply one.
- [ ] 4.2 Add `PlaceSearchField` between the label and the mode control, **with no `names` prop**,
...
- [ ] 4.3 Rename the middle mode and rewrite all three hint pairs to `design.md` §D1. These strings
      are contract and are asserted as written, as `064`'s were.
- [ ] 4.4 Offer the precise mode only when the photo carried a coordinate — **removed, not
      disabled**, per `064` §D8's rule applied one level down.
- [ ] 4.5 Extend the resolver in `src/lib/media/location.ts` to take the named place as well as the
      ...
      coordinate here.
- [ ] 4.6 **BLOCKED ON `Q1`.** Prefill the input from the photo: one lookup per file, none for a
...
- [ ] 4.7 Clear the mode **and** the location on a new file, per §D11. The mode reset already
      exists; extend it and extend its comment to say why the name goes too.
- [ ] 4.8 Pass no `recents` — §D12.
- [ ] 4.9 Render the hidden inputs from the resolver only, and none at all under `Hide`. Assert
      the absence, not the emptiness.
- [ ] 4.10 `Q4`: two labels or one. Ship the two-label version pending an answer.

## 5. Validation and the action

- [ ] 5.1 `src/lib/validation/postcards.ts`: `postcardPlaceNameSchema` (≤ 200, empty → null) and
      `postcardPlaceIdSchema` (≤ 512, empty → null). Zod carries the **message**; §1 carries the
      guarantee.
- [ ] 5.2 Widen `postcardLocationPrecisionSchema` to `['region', 'place', 'precise']` — `'region'`
      stays parseable because rows carry it, even though this client stops writing it.
- [ ] 5.3 Replace the coupling refine with one per arm of §1.6, so an honest client meets a field
...
- [ ] 5.4 `src/lib/actions/postcards.ts`: read the two new fields off `FormData` and name the two
      ...
      already travelled.
- [ ] 5.5 Unit tests for every arm, including the ones that must be **refused**: a `'place'` marker
...
- [ ] 6.1 The two whole-list grant pins for `postcards` INSERT and SELECT will fire on this
...
- [ ] 6.2 The `pg_get_constraintdef` pin for `postcards_taken_location_coupling` moves. Update it
      to the new definition; do not relax it to a `LIKE`.
- [ ] 6.3 `postcards_region_location_is_rounded` is **gone by name**. Grep the suite for it and
...
- [ ] 6.4 Assert each refused arm of §1.6 and each accepted one — including the **positive** at a
...
- [ ] 6.5 Assert the coarse-rounding constraint fires for `'place'` as well as `'region'`, and
      passes for a `'place'` row with NULL coordinates.
- [ ] 6.6 Assert UPDATE is still exactly `caption, club_id, image_path`, and that neither new
      column holds UPDATE for `authenticated`.
- [ ] 6.7 Assert `anon` holds nothing on either column in any verb, **naming the role** — a
...
- [ ] 6.8 Assert the visibility table in `specs/database-enforced-integrity`: author, club member,
...
- [ ] 6.9 Assert no policy on `postcards` moved — capture `qual`/`with_check` digests before
      applying and compare after.
- [ ] 6.10 Assert a legacy `'region'` row survives: insert one before the constraint swap in a
      rolled-back transaction and show it still satisfies every constraint after.
- [ ] 6.11 **Mutation-test at least two of these** rather than reading them green: drop the
...
- [ ] 7.1 Apply `072` to DEV. Re-read both grant lists and confirm **UPDATE has not moved**.
- [ ] 7.2 Confirm the four constraints exist by name and that
      `postcards_region_location_is_rounded` does not.
- [ ] 7.3 `get_advisors(security)` on DEV — this file adds no function, no view and nothing
      `security definer`, so the count must not move. Anything new means something landed that the
      file did not describe.
- [ ] 7.4 `PGPASSWORD=postgres npm test` green, and reconcile the assertion count by **label set**
      against `origin/development`, recording the new/relabelled split in `docs/HANDOFF.md`'s
      assertion row.
- [ ] 7.5 `npm run db:drift` — the repo, DEV and PROD agree on the chain.
...
- [ ] 8.2 Update `docs/reference/schema.md`'s `postcards` row: the two columns, the new grant
      ...
      as fact.
- [ ] 8.3 `npm run docs:check` — several claims in `CLAUDE.md` and `docs/HANDOFF.md` measure things
      this change moves.
