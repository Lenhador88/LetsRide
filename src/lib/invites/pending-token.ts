...
import { INVITE_TOKEN_PARAM, RIDE_JOIN_PATH } from '@/lib/routes'
 ...
 * anything, and who must land back on the ride they were looking at rather than
 * on `/postcards`.
 *
 ...
 *
 * **Sign-out clears it** (`signOut` calls `clearStashedInviteToken`), because a
 * token is both a trace of the rider and a credential, and
 * `client-session-storage`'s standing rule is that sign-out destroys every local
 * trace.
 *
 ...
 * row: rider A opens a link, abandons sign-up, rider B signs in in the same
 * tab, and an automatic claim would join **rider B** to a private ride they were
 * never told about — a claim that is perfectly valid at the database layer,
 * where the caller is authenticated, onboarded and unblocked and the token is
 * live. No policy, trigger or RLS assertion can see that; only the client
 * contract can. `RideInviteJoin` is where it is held, and its test is what
 * keeps it.
 *
 ...
 */
export const PENDING_INVITE_TOKEN_KEY = 'letsride.pendingInviteToken'
...
export function stashInviteToken(token: string): void {
  if (!rideInviteTokenSchema.safeParse(token).success) return
  try {
    store()?.setItem(PENDING_INVITE_TOKEN_KEY, token)
  } catch {
...
export function readStashedInviteToken(): string | null {
  ...
  try {
    value = store()?.getItem(PENDING_INVITE_TOKEN_KEY) ?? null
  } catch {
    return null
  }
  return value && rideInviteTokenSchema.safeParse(value).success ? value : null
}

export function clearStashedInviteToken(): void {
  try {
    store()?.removeItem(PENDING_INVITE_TOKEN_KEY)
  } catch {
 ...
 * sent straight back to `/rides/join?token=…`, which stashes it again — so the
...
export function takeStashedInviteToken(): string | null {
  const token = readStashedInviteToken()
  if (token) clearStashedInviteToken()
  return token
}

/**
 * `undefined` until something has resolved the token this page load, then the
 * token or `null`.
 *
...
export function getPendingInviteToken(): string | null | undefined {
  return snapshot
}
 ...
 */
export function adoptInviteTokenFromLocation(): void {
  if (typeof window === 'undefined') return

  const fromUrl = new URLSearchParams(window.location.search).get(INVITE_TOKEN_PARAM)
  if (fromUrl && rideInviteTokenSchema.safeParse(fromUrl).success) {
    ...
    window.history.replaceState(window.history.state, '', RIDE_JOIN_PATH)
    return
  }

  publish(readStashedInviteToken())
}
