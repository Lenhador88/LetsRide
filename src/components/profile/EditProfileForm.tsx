...
export function EditProfileForm({ profile }: { profile: Profile }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4 px-6">
      ...
      {/* `sent` distinguishes "saved, nothing to report" from "not submitted",
          which are both `error: null`. See ActionState. */}
      {state.sent && !state.error && (
        <p role="status" className="text-center text-sm font-medium text-accent-strong">
          Saved
        </p>
      )}
    </form>
