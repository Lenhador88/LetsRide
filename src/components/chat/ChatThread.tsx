...
import { KebabIcon, TrashIcon } from '@/components/icons/generated'
import { ContextMenu, ContextMenuItem } from '@/components/ui/ContextMenu'
import { cn, formatChatMessageDay, formatRideTime } from '@/lib/utils'
...
export function ChatThread({
  ...
  // One menu for the whole thread rather than one per bubble: a list of two
  // hundred messages must not mount two hundred dialogs to draw one.
  ...
  return (
    ...
    <div className={cn('flex flex-1 flex-col overflow-y-auto px-2 py-4', className)}>
      ...
      {messages.map((message) => (
        ...
        >
          {message.startsDay && (
            // Not in the design — see `formatChatMessageDay` for why it is added
          ...
          {/* `contents` when there is no control, so a stream that offers no
              ...
              not this rider's to take down. */}
          <div
            className={
              onDeleteMessage && message.mine && !message.pending
          ...
          >
            {onDeleteMessage && message.mine && !message.pending && (
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted transition-colors active:bg-border"
              >
            ...
            >
              ...
              >
                {/* Reuses the ride's time formatter rather than getting a
      ...
      {onDeleteMessage && menuFor && (
        <ContextMenu open onClose={() => setMenuFor(null)} label="Message options">
          <ContextMenuItem
