...
import type { ClubDiscussionListItem } from '@/types'

/**
 * One thread in a club's Discussions list (`081`, PD-307).
 *
 * **There is no v2 Discussions frame, so this composition is ours** and is
 ...
 * - **The counter is a dot, not a number.** `club_discussion_unread` answers a
 ...
 */
export function ClubDiscussionRow({
  discussion,
  hasUnread,
}: {
  discussion: ClubDiscussionListItem
  /** From `clubs.discussionsUnread`. `false` for a thread with nothing new **and**
   * for a failed unread read — the mark decorates a row that works without it. */
  hasUnread: boolean
}) {
  return (
    <Link
      href={routes.clubDiscussion(discussion.id)}
      ...
      aria-label={
        hasUnread ? `${discussion.title}, unread messages` : discussion.title
      }
    ...
    >
      ...
      <span aria-hidden="true" className="flex min-w-0 flex-1 flex-col">
        ...
        <span className="truncate text-sm font-medium text-muted">
          Started by {discussion.author?.username ?? 'a rider'}
        </span>
      </span>
