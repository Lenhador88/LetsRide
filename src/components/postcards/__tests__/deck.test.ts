...
describe('armsDrag', () => {
  ...
  /**
   * A version measuring TOTAL travel (`Math.hypot(dx, dy)`) was written and
   * reverted before shipping — see `armsDrag`. This pins the reverted shape:
   * vertical drift is not travel as far as arming is concerned, because 8px of
   * it is inside ordinary touch slop and would swallow taps on the four
   * controls the threshold exists to protect.
   */
  it('ignores vertical drift, which is what keeps a control tappable', () => {
    expect(armsDrag(2)).toBe(false)
