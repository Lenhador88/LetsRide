...
**Applied state: 45 files, and DEV is FIVE AHEAD of PROD — DEV at `045`, PROD at `040`.** Do not
read that number here — it has been wrong in both directions. Run `list_migrations` against
`ls supabase/migrations/` instead. `041`–`045` are applied to DEV only, deliberately; promoting
them is the owner's call. **They are no longer all independent** — `044` grants
`insert (… ride_id)`, a column `041` adds, so `041` must precede `044`; `docs/HANDOFF.md`
§Migrations carries the full required order and what each one needs. **`044` and `045` are the two
with a PROD consequence while they wait**: PROD is where riders are, and until they apply there an
author can still pin their own postcard to the top of every feed, and a club owner their club to
the top of Explore, by writing `created_at`.
...
Suite **1173** assertions — re-derive rather than trust it:
