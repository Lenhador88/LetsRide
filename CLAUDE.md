...
```bash
# 4 — auth.ts:1 + onboarding.ts:3. The pathspec EXCLUDES __tests__, which is the
# whole point: two test files also match, and `-- src/lib/actions/` prints four
# LINES whose counts sum to 7 while looking like the answer.
git grep -c "invalidateOnboardingState()" -- 'src/lib/actions/*.ts' \
  | grep -v __tests__ | awk -F: '{n += $2} END {print n}'
```
...
**Applied state: 113 files. DEV is at `113` and PROD at `107` — measured 2026-09-07.** `101`–`107`
**promoted to PROD on 2026-09-06**, so the long-standing seven-file gap this line used to describe
is closed. What is open is the ordinary six-file promotion gap, `108`–`110` (PD-402), `111`
(PD-361), `112` (PD-399/PD-408) and `113` (PD-428), all applied to DEV. **`113` is
MIGRATION-FIRST and its partner `114` does not exist yet** — the country column has to be there
before the bundle that writes it, and the refusal that requires it must not exist until that
bundle is *serving*, so the change is split rather than compromised. `108`/`109` is the same
shape and the same rule: one file cannot be both sides of a deploy. **`111` and `112` are additive with nothing
