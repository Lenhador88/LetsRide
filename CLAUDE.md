...
**Applied state: 104 files. DEV is at `104` and PROD at `100` — measured 2026-09-03. The gap is
`101`, `102`, `103` and `104`, all awaiting promotion.** `103`/`104` (PD-103) carry an **ordering
rule for that promotion and it breaks in one direction only**: deploy the code first, then apply
`103`, then `104`. Applying `103` against a bundle that still writes the creator's membership row
is an instant outage of club and ride creation; the reverse gap is self-healing, because `103`'s
backfill repairs exactly the orphans it can leave. Count rather than trust it: `list_migrations` against both refs,
...
Suite **3381** assertions — re-derive rather than trust it:
