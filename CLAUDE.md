...
reads 102 against 99 files and `db:drift` will name them; every file IS applied, which is the
direction that matters. **PROD's recorded versions are NOT in filename order and that is
...
**Security advisors: thirty-seven on DEV and thirty-six on PROD, with identical name sets apart
from that one, and only one is outstanding** — measured 2026-09-01 after the `097`–`099` merge.
