...
against `ls supabase/migrations/*.sql | wc -l`. **DEV also records three rows with no file** —
...
reads 103 against 100 files and `db:drift` will name them; every file IS applied, which is the
direction that matters. **`097`–`100` all went MIGRATION-FIRST, before the build served** — the
same side `096` took and the opposite of `092`–`095`, and the reasoning is per file rather than
per batch: `097` is inert to an older bundle, `098` adds a column the bundle READS through an
explicit `NOTIFICATION_SELECT` column list (so a newer bundle against a pre-`098` database takes
every rider's notifications screen down), and `099` and `100` change no schema at all — `100`
narrows `098`'s two fan-outs and **must never be applied without it**. That reversed what PD-367
and `docs/HANDOFF.md` both said, because `089`'s exhaustive-switch premise expired one day after
`089` shipped: PD-335 gave both switches a self-healing runtime fallback. **PROD's recorded versions are NOT in filename order and that is
...
**Security advisors: thirty-seven on BOTH projects, with identical name sets, and only one is
outstanding** — measured 2026-09-01 after the `097`–`100` promotion.
Re-derive rather than trust the number — `get_advisors(security)`, or, cheaper and without the
enormous payload,

```sql
select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public' and p.prosecdef
   and has_function_privilege('authenticated', p.oid, 'execute');   -- DEV 34, PROD 33
```

— but the *shape* is durable, because all but one are
