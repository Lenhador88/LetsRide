...
## STEP 0.5 — Is this session idle? If not, exit.
...
**Exit silently if any of these is true.** No comment, no notification, no changes.
...
4. **A PR opened from this session is still open** — check with
   `mcp__github__list_pull_requests` on `Lenhador88/LetsRide`, state `open`. *Committed and
   pushed is not shipped*; an open PR is unfinished work, not finished work.

```bash
git status --porcelain                                   # (2) must be empty
git rev-parse --abbrev-ref HEAD                          # (3) expect development
...
is the worst possible place to guess. Exit and let the next hour try again.
...
- The steps up to and including STEP 3 are cheap: a handful of Linear calls and four git
  commands. Do those inline.
- **From STEP 4, the build runs in subagents** — `CLAUDE.md` §The Agent Squad pre-authorizes
  the squad, and the *reviewer* pass is non-negotiable regardless. A subagent's file reads,
  greps and test output never enter this conversation; only its conclusion does.
...
**If the conversation is already long when you arrive, run the entire pickup — STEP 2
onward — inside one subagent** and report only its result here. That bounds a firing's cost
to a few hundred tokens in the main thread instead of tens of thousands.
...
## STEP 1 — The lock. Do this before touching anything.

List the issues **of project `88f3f224-ecf0-46f0-a032-c86b7a12f81c`** and check their
statuses. **If ANY of them is in `Development (AI)` or `Needs help`, STOP IMMEDIATELY.**
Change nothing, comment nothing, end the turn. One story at a time is the rule — an issue in
either status means a previous firing is still in flight, or is blocked waiting on the owner.
...
blocked one looks stuck. Just exit.

**One exception to the silence, and it exists because a held lock is otherwise invisible.**
Check how long the issue has held that status — `mcp__Linear__get_issue` returns
`stateHistory[].startedAt`. **If the longest-held one has been there more than 3 hours but
less than 4, send a single push notification naming it and saying the queue is stalled, then
stop.** The window is narrow on purpose: it fires roughly once rather than every hour.
Outside it, exit silently as above.

This matters because the lock can be held by an issue nobody is working on — dragged there
by hand, or left behind by a session that died. Then the queue freezes for ever and every
firing exits quietly, which is the same failure STEP 0 exists to prevent.
...
Note the interaction with STEP 0.5: parking into `Needs help` leaves a branch and an open PR
behind on purpose, so the *next* firing will exit on the in-flight checks as well as on the
lock. That is two gates agreeing, not a bug.
