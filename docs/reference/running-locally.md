...
Under `environment: 'node'`, all but **ten** render through `renderToStaticMarkup`, and jsdom is
...
because each of the ten below is there for a different one. Count them with
...
- `ClubMemberRail.dom.test.tsx` (PD-382) — a real click: the rail's panel is `{open && …}` and
  ...
  every claim about what a failed rail *opens* would pass against one that opens nothing.
