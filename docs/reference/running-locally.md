...
- `rides/__tests__/filterbar-survives-tap.dom.test.tsx` and
  `postcards/__tests__/filterbar-survives-tap.dom.test.tsx` (PD-223) — the assertion is **element
  identity across a navigation**: the same `<nav>` node before and after a filter tap, which is
  only expressible against a live DOM. A static render produces two unrelated strings and cannot
  tell a surviving element from a remounted one, which is the entire defect. They mount the real
  pages and the real `@/lib/query`, mocking only the fetchers and the heavy siblings.
