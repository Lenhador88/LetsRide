...
curated rider (`sofiarides`) and four supporting ones: two clubs with covers and a discussion,
four rides with real map tiles, seven postcards with images, likes and comments. It exists because
DEV's accumulated walk residue is not photographable and the reset seed above cannot run here.

```bash
PGPASSWORD=... psql "$DEV_DATABASE_URL" -v ON_ERROR_STOP=1 \
  -v seed_password="$(openssl rand -base64 18)" -f supabase/seeds/screenshot-account.sql
NEXT_PUBLIC_SUPABASE_URL=... NEXT_PUBLIC_SUPABASE_ANON_KEY=... SEED_PASSWORD=... \
  node scripts/dev/seed-screenshot-media.mjs      # objects and tiles; --preview <dir> draws only
```

**Sign in as `sofia@letsride.dev`** — username `sofiarides`. The password is whatever
`-v seed_password` was, so it lives in PD-448's comment rather than here or in `docs/HANDOFF.md`,
whose carve-out covers walk fixtures alone. Changing it is one statement:
`update auth.users set encrypted_password = extensions.crypt('<new>', extensions.gen_salt('bf'))
where email like '%@letsride.dev' and id::text like '5c0f1a00-%';`
...
**A meeting point must be a street address or the ride gets no tile.**
`resolve-ride-location`'s first gate is a street-level `result_type`, so "Apeldoorn, Netherlands"
...
**The dispute is SETTLED — both baselines were measured on 2026-09-06 (PD-390), by running it.**
Neither number in the old argument was right, which is why the argument could not be won on paper:
...
**The SCREENS half of both rows is superseded by PD-426 (2026-09-07), which deleted two walked
routes** — `/clubs/detail/threads` and `/rides/detail/threads`. Both were in the route list for
either account, so **both rows lose 2**. Measured on the named account that day, `WALK_FIXTURES=1`,
green and exit 0: **24/24 screens, 81/81 checks**. A third route joins when the data allows it —
`/rides/detail/thread` needs a ride thread to exist, and `ride_threads` was EMPTY across DEV until
...
**PD-432 (2026-09-20) makes that 25th screen the walk's own doing rather than a row somebody left
behind**, and the named pair was re-measured on the same tree: **25/25 screens, 89/89 checks**,
exit 0, `WALK_FIXTURES=1`. The seed moved out of `provision()` — which only runs for what is
*missing* — into `seedRideThread`, called on the ride the walk will actually open, created or
reused. So the sentence above no longer describes a precondition to wait for.

**Both branches were run, and the second is the one worth keeping.** With the hand-left
`Walk fixture thread` still present the seed took its early return, printed nothing and left
`ride_threads` at 3 — the idempotence that keeps a shared DEV from silting up. That row was then
deleted and the walk re-run: it printed `+ seeded a thread on the fixture ride`, `ride_threads`
came back to 3, and `/rides/detail/thread` walked against the NEW thread id.

**The second run printed `17/25` and exit 1, and that is the quota trap above, not a regression.**
All eight unclean screens returned **200 and rendered**; each carried one `429` console line from
`search-places`, and `place_search_attempts` for `walkfixture` read **20 in the last hour** — the
documented per-rider ceiling exactly, reached by running two named walks back to back. The check
half was **89/89 on both runs**, which is the tell: no check reads a place lookup, so a real
regression could not leave that number untouched while the screen count fell.
...
`429 (Too Many Requests)` as a console error and were counted unclean — `17/25` — while **every one
of them still returned HTTP 200 and rendered**. The 429 is
`/functions/v1/search-places`, the Geoapify-backed typeahead, not Supabase and not the relay (which
has no rate limiter; direct probes answered 400/401, never 429). Capture the URL rather than
guessing at it — a `page.on('response')` filter for `status() === 429` over `/rides`, `/clubs`,
`/profile` is enough. **A pause of two minutes did not clear it**, so budget one walk per change
rather than iterating on it, and read a sudden crop of 429s as quota rather than as a regression. **These are the walk's own printed totals**, not a reconstruction — the whole point
...
`/clubs/detail/thread`"* is not what separates these two totals.

**The historical numbers this replaces, so nobody re-derives them:** `docs/HANDOFF.md`'s `23`/`44`
named and `22`/`47` minted (2026-09-01/02), and #390's `56/56` on `walk-fixture` read as `47 + 9`.
The screens figure has moved a long way since — PD-358's two landing routes and PD-402's three
thread routes are in the 26 — so none of those is a base anything may be added to.

**The per-change deltas below are kept as history, not as arithmetic to perform.** They were this
section's whole answer while the base was unknown; with both bases measured above, adding to a
remembered number is the thing to stop doing. Re-run the walk instead — it takes about four
minutes.
...
2026-08-12 — five phases and five routes ago. Do not compare a run against any of them.

**The account that measures the full total
...
last run *for `077`'s question*, and it is NOT the walk's last run.** That is **2026-09-06**, the
measured pair at the top of this file; a reader taking this heading literally reconstructs a
superseded baseline, which is the failure that section is about. Kept because what it answered is
still worth having: `tsc`, ESLint,
...
2026-08-12 with a ride, a club and one visible postcard. **Those three are 2026-08-12 figures kept
to show the shape, not a base** — five routes have been added since and the current measured pair
is **26** screens for both accounts (top of this file). **Read the `N/N` for equality, not for
...
Under `environment: 'node'`, all but **twenty** render through `renderToStaticMarkup`, and jsdom is
...
because each of the twenty below is there for a different one. Count them with
`git grep -l "@vitest-environment jsdom" -- 'src/**/*.test.tsx'`.

- `rides/__tests__/filterbar-survives-tap.dom.test.tsx` and
  `postcards/__tests__/filterbar-survives-tap.dom.test.tsx` — **element identity across a
  navigation**: the same `<nav>` node before and after a filter tap. A static render produces two
  unrelated strings and cannot tell a surviving element from a remounted one. **They gate PD-210's
  collapsed gate, NOT the blink PD-223 reports** — that one is a failed RSC fetch making
  `next/link` hard-reload the document, which a mocked `next/link` can never do, so do not widen
  these files trying to catch it. Each also asserts the tapped tile ends up `aria-current`, without
  which the whole file passes vacuously against a navigation that never fired.
...
- `LocationQuestionRow.dom.test.tsx` (PD-419, rewritten by PD-447) — an **event** and a
  **portal**: the row's tap opens a `ContextMenu` that portals to `document.body`, and the
  dismissal it records is only observable through that click. **No longer a timer** — PD-447
  deleted the automatic ask. Under `environment: 'node'` the permission read never resolves, so
  the component renders `null` for ever and every assertion would pass against one that does
  nothing.
- `ClubMemberRail.dom.test.tsx` (PD-382) — a real click: the rail's panel is `{open && …}` and
  `open` is local state, so under `environment: 'node'` there is no panel to assert against and
...
- `use-swipe-back.dom.test.tsx` (PD-341) — **listeners**, which is where the whole defect was: the
  hook has to call `preventDefault` on a non-passive `touchmove`, and only for a gesture it already
  admitted. The pure module beside it had every threshold right while the gesture never fired on a
  phone. jsdom cannot pan, so it pins the CALL; the PR carries the CDP measurement.
