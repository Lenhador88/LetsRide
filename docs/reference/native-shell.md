...
| 7 | ~~**Signup never exercised end to end**~~ — **the app's confirmation-on arm has now RUN, 2026-08-27/28 (`PD-252`); the AUTH SERVER was proven 2026-08-16 (`PD-91`)** | **Not "proven" without its two boundaries, and both matter.** (1) **The DEPLOYED BUNDLE is still unexercised, and the reason this row used to give is gone** — `app.letsride.social:443` was refused by the agent proxy (`403` to `CONNECT`) until the owner opened the network policy on 2026-09-20, and it answers now, so do not read this row as "a session cannot reach production". What ran is still the app's own code on a local dev server pointed at PROD through the relay, and the obstacle to driving the real bundle is now `scripts/supabase-relay.mjs`'s: Chromium stalls on Supabase inside the browser rather than at the proxy, and an inlined build-time Supabase URL cannot be pointed at the relay. (2) **A delayed click is unmeasured**: four `confirm` runs within ~1–2.5 minutes of the mail were green, one at ~5 minutes failed inside `exchangeCodeForSession` with GoTrue clean, and the experiment that would settle it could not be completed here — `PD-337` holds it. What *is* established: `PD-91` used six raw HTTP calls to GoTrue, so `signUp` itself never ran; `scripts/probes/signup-confirmation.mjs` closed that at **11/11, 0 residue**. The `!data.session` arm (`src/lib/actions/auth.ts`) and its "Check your email" screen (`src/app/auth/signup/page.tsx`) have executed, and the emailed link lands the rider signed in on `/onboarding/terms`. **DEV structurally cannot cover them, measured rather than read off decision #6** — `/auth/v1/settings` reports `mailer_autoconfirm` **True** on `fpmrimzxadewsaiwpsel` and **False** on `zwprydcyryvudhurbnye`. The *automated* check is `PD-334`'s decision; `docs/reference/signup.md` §Signup has the mechanism |

| 8 | **The App Privacy label — answerable now, and not yet answered. Superseded 2026-09-18 by PD-455 and PD-456** | This row used to say the label was wrong because three third-party SDKs had arrived and one of them filmed the rider's screen unmasked. **Both halves moved.** PD-456 set `disable_session_recording: true`, so there is no replay to declare — the honest answer is now crash diagnostics plus product analytics. PD-455 added `ios/App/App/PrivacyInfo.xcprivacy`, which carries the measured list: `NSPrivacyTracking` false, `NSPrivacyAccessedAPITypes` **empty** (the app target's own Swift is `AppDelegate` and `SceneDelegate` and touches no required-reason API — re-run the grep in that file's header before adding Swift), and **eleven** collected-data types, every one Linked and none Tracking — count rather than trust it, `grep -c "<key>NSPrivacyCollectedDataType</key>" ios/App/App/PrivacyInfo.xcprivacy`. **What is still owed is the App Store Connect questionnaire itself and Play's Data safety form**, which are the owner's at submission — the manifest is the list to fill them from rather than a substitute for them, and the Apple half is now transcribed field-by-field in `docs/reference/app-store-listing.md` §App Privacy. Two facts a submission must not infer: the rider's `auth.uid()` IS sent to both processors (deliberately — `src/lib/observability/scrub.ts` has the argument), and **never** their email or username. `docs/reference/observability.md` has the per-field table |
