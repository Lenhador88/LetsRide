...
- ~~**`createClub` and `createRide` can leave a club with no owner row, or a ride whose organizer
  is not on its own crew.**~~ **FIXED by `PD-103` — `103_creator_membership.sql` + `104_club_member_owner_arm.sql`, applied to DEV 2026-09-04 after the merge was confirmed serving. Live on DEV; PROD still has neither.** The
  ...
  **Still open, and deliberately:** `PD-194`'s ownership transfer shipped separately in `095`, so
  ...
  yet** — PROD has neither, so the ratings below are for that promotion, which is all that is
  left of this entry. **Check, do not trust:** `list_migrations` on both refs.
