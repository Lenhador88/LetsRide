...
npm run test:unit                     # 2872/2872 across 99 files
...
PGPASSWORD=postgres npm test          # 3035 assertions, 0 failures
```
...
| `096` | On DEV. **Additive, so it applies to PROD BEFORE the build serves** — build-first gives `sendFeedback` a `PGRST204` on a column that does not exist and takes feedback submission down entirely. No client ordering constraint | The promotion, with `092`–`095` in filename order |

**`092` has a client-side prerequisite that its own ordering note does not cover, and PROD is
one promotion away from it.** `club_join_waves` gives PostgREST a second `club_members`↔`profiles`
relationship, so every unhinted embed of that pair answers `PGRST201` / HTTP 300 the moment the
migration applies — which is what took Your clubs, Explore clubs, the club roster and the club
timeline down on DEV between `092` applying and this fix. **PROD is at `091` and is therefore fine
today**, and it breaks identically if `092` reaches it before the bundle carrying
`MEMBER_PROFILE_EMBED` is serving. So `092` is additive in schema and **client-ordered like `089`**:
confirm the promotion's Vercel deployment `READY` on the merge sha with `aliasError` null, then
apply. Re-derive rather than trusting this line — it is one `curl` and needs no session:
...
ls supabase/migrations/ | wc -l          # 96 — DEV at 096, PROD at 091: 092-096 await promotion
```
