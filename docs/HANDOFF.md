...
npm run test:unit                     # 2438/2438 across 77 files
NEXT_PUBLIC_SUPABASE_URL=https://placeholder.supabase.co \
  NEXT_PUBLIC_SUPABASE_ANON_KEY=placeholder npm run build   # exit 0, 39 static routes
node scripts/native/assert-web-build.mjs   # that build was the web app, not the bundle
PGPASSWORD=postgres npm test          # 2279 assertions, 0 failures
```
...
`next build` reports **39 static** and **0 dynamic**, and no `ƒ Proxy (Middleware)` line appears
at all. Do not read the `Generating static pages (40/40)` line as the static route count — it is a
...
ls supabase/migrations/ | wc -l          # 87 — DEV at 087, PROD at 079
```


**PROD is FIVE behind, and the order inside the gap is not optional.** `080`, `081`, `082`, `083`
then `084`, in filename order, and **all five before the promotion build serves** — every one is
additive. Two of them carry a reason beyond the ordering rule:
...
**Gone from BOTH projects as of 2026-08-19:**
...
`locality_centroid()` — DEV first, PROD once the promotion build was confirmed serving there.
Retired
