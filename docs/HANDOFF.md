...
npm run test:unit                     # 2011/2011 across 64 files
...
PGPASSWORD=postgres npm test          # 1657 assertions, 0 failures
```
...
## Migrations — the repo holds 73, DEV is at `073` and PROD at `070`

**`list_migrations` prints 75 on DEV and 70 on PROD against 73 files, and none of the five gaps is
...
as the one consolidated file. The other three are `071` (`rides(departure_at)`, applied to DEV
2026-08-19) and `072`/`073` (the postcard's named place, PD-275, applied 2026-08-20), all simply
...
FALSE, and **a CHECK accepts NULL**, so `072` alone admits a coordinate with no marker, which is
the exact shape `064`'s own assertion exists to refuse. Promoting `072` without `073` puts that
hole on PROD.
...
ls supabase/migrations/ | wc -l          # 73 — 071/072/073 are DEV-only, PROD is at 070
```
