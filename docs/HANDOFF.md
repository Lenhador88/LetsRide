...
- **Xcode Cloud (PD-474) delivered `1.0.0 (2)`** — the 2026-09-23 promotion (`c6f64afd`), the
  first build carrying both gesture fixes. It is **installed on the iPad and not on the phone**:
  the iPhone 17 holds `1.0.0 (1)`, which predates both.
- **`External testers` exists and can deliver nothing** — Xcode Cloud's post-action names
  *LetsRide internal* alone, so an external build is attached by hand and waits on Beta App
  Review. Dave Kok is internal and installs now; Florina's App Store Connect invitation is
  unaccepted, and that is what keeps her out.
...
**`1.0.0 (2)` on the iPhone.** It carries both gesture fixes and neither is verified on
WKWebView — they were measured in Chromium. Check the postcard pull-down and
