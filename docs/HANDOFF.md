...
npm run test:unit                     # 1717/1717 across 55 files
...
PGPASSWORD=postgres npm test          # 1670 assertions, 0 failures
```
...
## Migrations — the repo and DEV hold 63, PROD holds 59
...
current crew is allowed, evicts nobody, and leaves a legal over-subscribed ride) and that the count
**excludes the row's own subject**, because `setRideAttendance` upserts and a `BEFORE INSERT`
trigger fires on an upsert that resolves to an `UPDATE` — a plain `count(*)` would freeze every
existing member's RSVP on a full ride.
...
ls supabase/migrations/ | wc -l          # 63
```
