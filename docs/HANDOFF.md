...
npm run test:unit                     # 2171/2171 across 70 files
...
**Documents tracks the `Generating static pages (N/N)` line rather than the `34 static` one**, and
the two differ for reasons that currently cancel: `check-export.mjs` walks `out/` and counts every
emitted `.html`, so it picks up `/_not-found`, which the route table omits — and it does not count
`/icon.png`, which the route table has and which emits no document. **So documents equalling the
route count today is a coincidence of those two, not a rule**: add one page and both move, add one
icon convention and only the route count does. **Do not pin the total**: two builds of the same commit came back 384
