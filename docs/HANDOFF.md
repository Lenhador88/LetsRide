...
npm run test:unit                     # 2265/2265 across 73 files
...
PGPASSWORD=postgres npm test          # 1816 assertions, 0 failures
```
...
A build with the wrong backend or the wrong origin baked into it cannot be repaired by a deploy
(PD-188), so this is the only thing that can stop one. `public/app-version.json` carries a
`minimum`; `src/lib/native/version-gate.ts` fetches the **deployed** copy from
`canonicalOrigin()` once per launch, on native only, and `UpdateGate` in the root layout replaces
the whole app when this build is below it. **It fails open on everything** — offline, timeout,
...
build known broken and nothing else. Android deep-links Play from the `appId`; **iOS shows
...
```bash
grep -n '"minimum"' public/app-version.json
npx vitest run src/lib/native/__tests__/version-gate.test.ts src/lib/__tests__/version.test.ts
```
