...
npm run test:unit                     # 1670/1670 across 53 files
...
- **`getItem` resolves to `null` on a storage failure.** `auth-js`'s `__loadSession` is
  ...
  ```bash
  grep -c "catch" src/lib/auth/guard-cache.ts   # was 0
  ```
