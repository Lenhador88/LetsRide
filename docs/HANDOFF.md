...
npm run test:unit                     # 2147/2147 across 70 files
...
PGPASSWORD=postgres npm test          # 1797 assertions, 0 failures
```
...
## Migrations — the repo holds 78, DEV is at `078` and PROD at `077`

**`list_migrations` prints 80 on DEV and 77 on PROD against 78 files, and only ONE of those two
...
survive. **The real gap is `078`** (PD-301, `push_devices`), applied to DEV on 2026-08-25 and
owed to PROD — additive, so it goes to PROD *before* the promotion build serves.
Repo 78 files, PROD 77 rows, DEV 80 rows, one chain, one file owed.
...
ls supabase/migrations/ | wc -l          # 78 — DEV is at 078, PROD at 077
```
