...
npm run test:unit                     # 2319/2319 across 74 files
...
PGPASSWORD=postgres npm test          # 1840 assertions, 0 failures
```
...
at all. Do not read the `Generating static pages (35/35)` line as the static route count — it is a
...
## Migrations — the repo holds 80, DEV is at `080` and PROD at `079`

**`list_migrations` prints 82 on DEV and 79 on PROD against 80 files, and only ONE of those two
...
**The real gap is `080`** (PD-193, a ride's meeting-point timezone), applied to DEV 2026-08-26 and
**owed to PROD at the next promotion**. It is ADDITIVE — a nullable column, a trigger, two
column grants — so it goes to PROD **before** the promotion build serves, which is the `069` half of
the `069`/`070` rule rather than the `070` half. Nothing breaks if it lands late: the column is NULL
on every row, and NULL is the fallback that every ride already had.

Repo 80 files, PROD 79 rows, DEV 82 rows, one chain, **one promotion owed**.
...
ls supabase/migrations/ | wc -l          # 80 — DEV at 080, PROD at 079
```
