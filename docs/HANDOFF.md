...
PGPASSWORD=postgres npm test          # 3301 assertions, 0 failures
```
...
```bash
ls supabase/migrations/*.sql | wc -l    # 99
```
```
mcp__Supabase__list_migrations zwprydcyryvudhurbnye   # PROD — 96 rows, last `096_analytics_opt_out`
mcp__Supabase__list_migrations fpmrimzxadewsaiwpsel   # DEV  — 102 rows, last `a_club_thread_notifies`
```
...
ls supabase/migrations/*.sql | wc -l     # 99 — DEV at 099, PROD at 096: 097-099 await promotion
```
