...
npm run test:unit                     # 1655/1655 across 53 files
...
PGPASSWORD=postgres npm test          # 1507 assertions, 0 failures
```
...
| Unit tests | `npm run test:unit` — **1655 across 53 files on a clean tree**. **One new file under `src`/`scripts` is worth +2 here, not +3** — measure it rather than counting the scanners, because the obvious count is wrong:

```bash
echo "export const probe = 1" > src/lib/__probe.ts
npx vitest list --run | grep -c " > " ; rm src/lib/__probe.ts    # +2
```

**Two** suites run `it.each` over the walked file list — `no-service-role-key.test.ts` and `no-geoapify-key.test.ts`. `use-server-exports.test.ts` walks `src/` too but emits **two fixed cases** whatever it finds ("is empty", "still checks any that come back"), so it does not move with the file count; its two `it.each` calls iterate literal fixtures. Reading it as a third per-file scanner is what made the deltas below look unreconstructible.

With that, each move is exact: PD-235 **+18** = `auth-email-templates.test.ts`'s 16 cases + 2. Before it, PD-102 **+20**; the first reviewer pass **+2** (`src/lib/flags.ts`, the new `accountDeletionEnabled` gate — one new *source* file, so 0 + 2); and the delta re-review **+13** = `flags.test.ts`'s 11 cases + 2. New cases in `guard.test.ts`/`guard-cache.test.ts` for the `gone` GuardState and its local-destruction side effect, and a `no-service-role-key.test.ts` rewrite of "the body is never read" into "the body never supplies an id", which is what the D6 re-authentication arm made the old assertion claim falsely. **Do not read a rise as "tests were added"**: `no-service-role-key.test.ts` runs `it.each` over every scanned *source* file, so the count moves whenever a source file is added, not only a test. `registry.test.mjs` does the same over every `docs:check` claim, so adding one entry to `scripts/docs/registry.mjs` also raises this by one. It also moves for an **untracked scratch script**, so a leftover `scripts/.tmp-probe.mjs` reads one higher and looks like a gained test. Delete scratch files before quoting this, or the number measures your working tree rather than the suite |
