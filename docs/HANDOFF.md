...
npm run test:unit                     # 2549/2549 across 84 files
...
PGPASSWORD=postgres npm test          # 2340 assertions, 0 failures
```
...
`next build` reports **40 static** and **0 dynamic**, and no `ƒ Proxy (Middleware)` line appears
at all. Do not read the `Generating static pages (41/41)` line as the static route count — it is a
...
ls supabase/migrations/ | wc -l          # 90 — DEV at 090, PROD at 079
```


**PROD is ELEVEN behind, and the order inside the gap is not optional.** `080` through `088`, in
