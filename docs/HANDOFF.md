...
npm run test:unit                     # 2109/2109 across 68 files
...
PGPASSWORD=postgres npm test          # 1754 assertions, 0 failures
```
...
at all. Both numbers fell by one with `075` (PD-286), which deleted `/onboarding/location`.
...
`/auth/confirm` was added and by −1 when `075` deleted `/onboarding/location`. **Do not pin the total**: two builds of the same commit came back 384
...
## Migrations — the repo holds 76, DEV is at `076` and PROD at `075`

**`list_migrations` prints 78 on DEV and 75 on PROD against 76 files, and only ONE of the
three-row difference is a real gap.** Two of them are a recording difference: DEV's **two surplus
...
file. The third is `076` (PD-297), applied to DEV on 2026-08-24 and **not** to PROD, which is the
ordinary DEV-ahead state a migration lives in between its merge and its promotion. Repo 76 files,
PROD 75 rows, DEV 78 rows, one chain.
...
ls supabase/migrations/ | wc -l          # 76 — DEV is at 076, PROD at 075
```
