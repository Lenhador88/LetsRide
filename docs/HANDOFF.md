...
npm run test:unit                     # 1594/1594 across 49 files
...
PGPASSWORD=postgres npm test          # 1506 assertions, 0 failures
```
...
- **Account deletion's flow is built (2026-08-16, `PD-102`) and points at a function the deployed
  build has not caught up with.** `029`–`032` are applied. The re-authentication arm (D6/Q7) landed
  in `supabase/functions/delete-account/index.ts` as its own commit ahead of the client one — the
  ordering that keeps a redeploy fail-closed rather than fail-open — but **the deployed function on
  both projects still predates it**, same as before this session: no `supabase` CLI here, no MCP
  deploy tool, so an edit to that file is drift until the owner redeploys. `ProfileMenu`'s Delete
  account row opens a sheet (`DeleteAccountSheet`, not a route — `2303:9370` turned out to be a
  second `ContextMenu`-shaped overlay over `/profile`, not its own screen), the action distinguishes
  the function's `reauth_required` from its `unauthorized`, and a deleted account's session is now
  destroyed on any device that discovers it, not just the one that ran the deletion
  (`client-session-storage`'s `gone` GuardState). **Store blocker 2** — App Store 5.1.1(v) — moves
  from "flow not built" to "flow built, gate not live until the owner redeploys".

  What is still open: `2.4` (idempotency under concurrent deletions — unverified, no new work this
  session), `2.6` (the live exercise, owed again against the redeployed build — five cases plus a
  sixth for the reauth arm), `6.3` (the live walk), and two decisions that are the owner's/legal's
  ...
  second `delete-account` call with the same token still returns 401 as success**, and that is
