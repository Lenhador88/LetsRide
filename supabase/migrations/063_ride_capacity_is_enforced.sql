...
--     is a LEGAL state. The alternative — refusing the organizer's edit — takes
...
--    fires BEFORE INSERT and then BEFORE UPDATE. So on a ride exactly at its
--    cap, a rider ALREADY on the crew changing Going -> Maybe would be refused
--    by a plain `count(*) >= max_riders` — because that count includes their own
--    row. A full ride would freeze every existing member's RSVP, which is a
--    worse bug than the one this file fixes.
--
--    The count therefore EXCLUDES the row's own subject: `user_id <>
--    new.user_id`. That is correct in all three cases — a new joiner sees
--    `cap >= cap` and is refused; a joiner taking the last seat sees
--    `cap - 1 >= cap`, false, and lands; an existing member re-RSVPing sees
--    `cap - 1 >= cap`, false, and is allowed. It also makes the check
...
--    each see `cap - 1` and both land. `for update` on the parent `rides` row
--    is what makes the last seat singular, and the ordering is safe — a BEFORE
--    trigger runs before the FK's own `FOR KEY SHARE` lock is taken (foreign
--    key checks are AFTER-statement), so this is not the lock upgrade that
--    deadlocks. Do not "fix" it by moving the lock.
--
...
begin
  ...
  -- `<> new.user_id` is trap 1 in the header. Do not simplify it to count(*).
  ...
  if taken >= cap then
    ...
    raise exception 'this ride is full — % of % seats are taken', taken, cap
...
-- A trigger function; nothing should ever call it directly, and leaving it
-- executable would put a `security definer` function on the exposed PostgREST
-- surface for no reason. `022` and `023` do the same.
revoke all on function public.enforce_ride_capacity() from public, anon, authenticated;

comment on function public.enforce_ride_capacity() is
  'Caps a ride crew at rides.max_riders (063, PD-174). A JOIN GATE, not an invariant: lowering a cap below the current crew evicts nobody and an over-subscribed ride is legal. Counts every ride_members row, going and maybe alike, because "on this ride" already means that everywhere else in this schema. security definer because 009 put private.is_blocked on the ride_members SELECT policy, so an invoker-rights count is short by the caller''s blocks. Excludes the row''s own subject (user_id <> new.user_id) because a BEFORE INSERT trigger fires on an upsert that resolves to an UPDATE, which would otherwise freeze every existing member''s RSVP on a full ride. Takes for update on the parent rides row so two riders cannot both take the last seat.';
...
comment on column public.rides.max_riders is
  'Crew cap, 1..999 or NULL for no cap (018 bounds the value). ENFORCED since 063 by public.enforce_ride_capacity() on ride_members — as a join gate rather than an invariant: an organizer may lower it below the current crew size, nobody is evicted, and no further rider may join until the crew drops back under it. Counts rows of BOTH statuses.';
...
--
-- 4. Advisors: this adds NO new WARN. `enforce_ride_capacity` is
--    `security definer` but holds no EXECUTE for `authenticated`, so it is not
--    an `authenticated_security_definer_function_executable` — the same shape
--    `enforce_participation_gate` has had since 023, which is likewise absent
--    from that advisor's eight.
